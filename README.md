...
| `--ddtree-ghost-batch` | off | Preserve the 15-token chain on confident rounds and add one three-way ghost batch of 5-token paths only on uncertain rounds |
| `--ddtree-ghost-margin F` | — | Required with Ghost Batch DDTree. Launch the ghost batch when the first-continuation top-1/top-2 log-probability margin is below `F`; calibrate explicitly for the deployment hardware/workload |
...
Ghost-batch shape, exactness, telemetry, and mixed-backend validation guidance are documented in [`server/docs/DDTREE_GHOST_BATCH.md`](server/docs/DDTREE_GHOST_BATCH.md).
