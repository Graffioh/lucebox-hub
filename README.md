...
**Decode (DFlash + DDTree + QGBatching)**

| Flag | Default | Effect |
|---|---|---|
| `--ddtree` | off (chain) | Enable tree verify |
| `--ddtree-budget N` | `22` | Tree size. 22 on 3090 (default), 40 on 5090, re-sweep on GB10 |
| `--qgbatching` | off | Run one Quality Ghost Batching round for a greedy request ([design and limits](server/docs/QGBATCHING.md)) |
| `--qgbatching-ghost-branches N` | `4` | Hypothetical short continuations (2–8) |
| `--qgbatching-horizon N` | `7` | Tokens in each ghost branch |
| `--qgbatching-margin F` | `0.10` | Required target-score gain before replacing ghost branch 0 |
