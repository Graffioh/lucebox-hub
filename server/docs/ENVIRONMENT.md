...
| `GGML_CUDA_MLA_STREAM_F32_STAGE` | enabled for maskless ratio-4 sparse prefill; unset otherwise | With streaming D512 indexed attention, convert aligned F16 pairs once while staging them in shared memory instead of repeating conversion for every head. Set `0` to restore F16 staging. |
