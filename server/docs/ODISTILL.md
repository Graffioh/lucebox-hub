# ODistill: online drafter adaptation

Status: implemented behind `--odistill`. A bounded 2026-08-13 qualification ran
...
ODistill teaches the DFlash drafter from the target model's real verification
...
The target already computes the supervision ODistill needs. ODistill captures a
...
adapter. If no trainer executable is configured, ODistill runs in capture-only
mode.
...
[`RESULTS.md`](../../optimizations/odistill/RESULTS.md).
...
`--odistill-device cpu` and keep the same host-memory guards.
...
measurement of the ODistill split.
...
[trainer README](../../optimizations/odistill/README.md#quick-start). Require
...
```bash
mkdir -p server/models/odistill
hf download unsloth/Qwen3.6-27B-GGUF Qwen3.6-27B-Q4_K_M.gguf \
  --revision e41d24e0e6909d1e15f79445cd9ac27ced27724a \
  --local-dir server/models/odistill
hf download Lucebox/Qwen3.6-27B-DFlash-GGUF \
  dflash-draft-3.6-q4_k_m.gguf \
  --revision fca10ba135c9d834988e15b7d7f5aee8ebc562a7 \
  --local-dir server/models/odistill
sha256sum server/models/odistill/Qwen3.6-27B-Q4_K_M.gguf \
  server/models/odistill/dflash-draft-3.6-q4_k_m.gguf
# 41ae55b347988dca8352ed4c85f3d8ee3804a23cc89aaea165c071d61ec3cca0  server/models/odistill/Qwen3.6-27B-Q4_K_M.gguf
# e2500e90165a0f8e7b52c9882c29ed1fa391c60b300ff11b817bf10e31fa092e  server/models/odistill/dflash-draft-3.6-q4_k_m.gguf
```

Capture verification data without starting a trainer:

```bash
server/build/dflash_server server/models/odistill/Qwen3.6-27B-Q4_K_M.gguf \
  --draft server/models/odistill/dflash-draft-3.6-q4_k_m.gguf --draft-swa 2048 \
  --target-device hip:0 --draft-device hip:0 \
  --max-ctx 4096 --cache-type-k q4_0 --cache-type-v q4_0 \
  --ddtree --ddtree-budget 22 \
  --odistill --odistill-ring-mb 256 --odistill-topk 8
```
...
server/build/dflash_server server/models/odistill/Qwen3.6-27B-Q4_K_M.gguf \
  ...
  --ddtree --ddtree-budget 22 \
  --odistill \
  --odistill-device 1 --odistill-dtype auto \
  --odistill-ring-mb 512 --odistill-topk 8 \
  --odistill-trainer-bin optimizations/odistill/bin/odistill-trainer
```

The integrated engine passes the target GGUF to the trainer because the mirror
shares its output head and token embeddings. `ODISTILL_TARGET_GGUF`, `--target`,
...
published 2,048-token `[S,S,S,S,F]` behavior requires `--draft-swa 2048`. ODistill passes the
...
Adapter format v3 binds each file to the exact draft and target GGUF SHA-256 values plus the
complete resolved RoPE/SWA/mask contract. Engine-only draft debug overrides disable ODistill rather
...
`~/.lucebox/odistill/<drafter-hash>/<profile>-sem-<contract-hash>/` and the next session starts from
...
1. **Capture only.** Run the first command in Usage at `--max-ctx 4096`, with
   ...
   outputs versus ODistill off.
2. **Isolate the Strix allocation.** Keep that capture-only server running,
   copy its `/lucebox-odistill-<pid>` ring name from the log, and attach exactly
   ...
   ```bash
   ODISTILL_RING_NAME=/lucebox-odistill-12345  # paste the name from the server log
   optimizations/odistill/bin/odistill-trainer \
     server/models/odistill/dflash-draft-3.6-q4_k_m.gguf \
     --ring-name="$ODISTILL_RING_NAME" \
     --target=server/models/odistill/Qwen3.6-27B-Q4_K_M.gguf \
     --out-dir=/tmp/odistill-smoke --profile=smoke \
     --device=1 --dtype=auto \
...
3. **Integrated defaults.** Restart with the online command in Usage. Hold
   ...
   on Strix. Require
   `/props.odistill.trainer_alive=true`,
   `/props.odistill.training_disabled=false`, increasing `steps`, and bounded
...
to `--odistill-device cpu` explicitly.

| Flag | Default | Meaning |
|---|---:|---|
| `--odistill` | off | Enable capture and online adaptation support |
| `--odistill-device <cpu\|N>` | `1` | Device used by the trainer sidecar |
| `--odistill-dtype <type>` | `auto` | Mirror dtype; auto is FP16 on GPU and FP32 on CPU |
| `--odistill-profile <name>` | `default` | Independent persistent adapter profile |
| `--odistill-lora-rank <N>` | `16` | Fixed LoRA rank |
| `--odistill-alpha <F>` | `32` | LoRA scaling value |
| `--odistill-dir <path>` | `~/.lucebox/odistill` | Adapter and profile store |
| `--odistill-ring-mb <N>` | `512` | Shared-memory ring capacity in MiB |
| `--odistill-topk <K>` | `8` | Target top-K values captured; `0` disables them |
| `--odistill-trainer-bin <path>` | empty | Trainer executable; empty means capture-only |
...
Compare the same workload with ODistill disabled, capture-only and training
enabled. Record:
...
power and thermal measurements still do not exist, so ODistill remains an
implemented experiment rather than a general performance claim.

## Implementation map

- `server/src/common/odistill/` contains the ring, formats, supervisor, adapter
...
- `optimizations/odistill/` contains the Python trainer and its runtime package.
- `server/test/` and `optimizations/odistill/tests/` cover engine and trainer
