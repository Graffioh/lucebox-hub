...
The ROCmFP2 four-row MoE path defaults on for qtype-106 matvecs at q>2 on
gfx1151. Monolithic serving can reach it for every resident qtype-106 expert;
in the R9700-primary split, only the secondary Strix-owned expert branch can
reach it. q=1, q=2, gfx1201, and every non-gfx1151 target keep the two-row
launch. q=6 remains unreachable unless `DFLASH_DS4_BATCH_Q6_MMV=1` separately
widens the mixed-weight dispatch. Set `DFLASH_ROCMFP2_ROW4=0` before server
startup to restore two rows per wave. The runtime caches the setting on first
eligible dispatch, so restart the server after changing it.
