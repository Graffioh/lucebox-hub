...
TEST_CASE(GQBatchingFixture, builds_disjoint_short_branches) {
    ...
    const GQBatchingTree qtree = build_gqbatching_tree(ids, 3, 3, 3, 3);
...
TEST_CASE(GQBatchingFixture, builds_fully_conditioned_branch_paths) {
    const int32_t paths[] = {
        10, 11, 12,
        20, 21, 22,
        30, 31, 32,
    };
    const GQBatchingTree tree =
        build_gqbatching_tree_from_paths(paths, 3, 3);
...
TEST_CASE(GQBatchingFixture, rows_round_to_verify_tile) {
    CHECK_EQUAL(gqbatching_required_rows(4, 7), 32);
    CHECK_EQUAL(gqbatching_required_rows(2, 15), 32);
    CHECK_EQUAL(gqbatching_required_rows(4, 8), 64);
}

TEST_CASE(GQBatchingFixture, packed_draft_layout_isolates_branches) {
...
TEST_CASE(GQBatchingFixture, main_branch_wins_without_clear_improvement) {
    const int32_t ids[] = {1, 2, 3, 1};
    const GQBatchingTree qtree = build_gqbatching_tree(ids, 2, 2, 2, 2);
    auto logits = neutral_logits(qtree.tree.n_nodes + 1, 4);

    // Slightly prefer branch 1 at the root, but not by the configured margin.
    logits[2] = 0.05f;
    const auto selected = select_gqbatching_branch(
...
TEST_CASE(GQBatchingFixture, stronger_target_branch_is_selected) {
    const int32_t ids[] = {1, 2, 3, 1};
    const GQBatchingTree qtree = build_gqbatching_tree(ids, 2, 2, 2, 2);
    auto logits = neutral_logits(qtree.tree.n_nodes + 1, 4);
...
TEST_CASE(GQBatchingFixture, rejects_impossible_shape) {
    const int32_t ids[] = {1, 2, 3, 4};
    const GQBatchingTree qtree = build_gqbatching_tree(ids, 2, 2, 3, 2);
