// Unit tests for the ODistill engine pieces that need no GPU:
...
#include "common/odistill/odistill_adapter.h"
#include "common/odistill/odistill_config.h"
#include "common/odistill/odistill_format.h"
#include "common/odistill/odistill_guard.h"
#include "common/odistill/odistill_ring.h"
#include "common/odistill/odistill_runtime.h"
#include "common/odistill/odistill_supervisor.h"
#include "internal.h"
...
using namespace dflash::common::odistill;

struct ODistillUnitFixture {};
...
void write_adapter_file(const std::string & path,
    ...
    const auto specs = odistill_lora_expected_tensors(dw, rank);
    ...
    header["__metadata__"] = {
        ...
        {"odistill.rank", std::to_string(rank)},
        {"odistill.alpha", alpha},
        {"odistill.generation", std::to_string(generation)},
...
TEST_CASE(ODistillUnitFixture, format_header_layout_is_stable) {
    CHECK_EQUAL(sizeof(ODistillRingHeader), (size_t)256);
    CHECK_EQUAL(offsetof(ODistillRingHeader, head), (size_t)64);
    CHECK_EQUAL(offsetof(ODistillRingHeader, tail), (size_t)128);
    CHECK_EQUAL(offsetof(ODistillRingHeader, dropped_records), (size_t)192);
    CHECK_EQUAL(offsetof(ODistillRingHeader, drafter_hash), (size_t)208);
    CHECK_EQUAL(offsetof(ODistillRingHeader, target_hash), (size_t)240);
    CHECK_EQUAL(offsetof(ODistillRingHeader, drafter_semantics_hash),
                (size_t)248);
    CHECK_EQUAL(sizeof(ODistillRecordHeader), (size_t)32);
}

TEST_CASE(ODistillUnitFixture, hash_from_hex_truncates_big_endian) {
    CHECK_EQUAL(odistill_hash_from_hex("0123456789abcdef0000"),
                (uint64_t)0x0123456789abcdefull);
    CHECK_EQUAL(odistill_hash_from_hex("ABCDEF0102030405zzz"),
                (uint64_t)0xabcdef0102030405ull);
    CHECK_EQUAL(odistill_hash_from_hex("shorty"), (uint64_t)0);
    CHECK_EQUAL(odistill_hash_from_hex(nullptr), (uint64_t)0);
}

TEST_CASE(ODistillUnitFixture, conservative_runtime_defaults) {
    ODistillConfig cfg;
...
TEST_CASE(ODistillUnitFixture, adapter_generations_must_advance_resident_and_pending) {
    CHECK(odistill_generation_is_newer(8, 7));
    CHECK(!odistill_generation_is_newer(7, 7));
    CHECK(!odistill_generation_is_newer(6, 7));
    CHECK(odistill_generation_is_newer(9, 7, 8));
    CHECK(!odistill_generation_is_newer(8, 7, 8));
    // A rollback may lower the resident generation, but must not make a
    // previously seen/rejected generation eligible again.
    CHECK(!odistill_generation_is_newer(8, 7, 0, 8));
    CHECK(odistill_generation_is_newer(9, 7, 0, 8));
}

TEST_CASE(ODistillUnitFixture, runtime_refuses_incompatible_feature_width_before_gpu_init) {
    ODistillRuntime runtime;
    ODistillConfig cfg;
    ...
    CHECK(!runtime.init(cfg, "/tmp/target.gguf", "/tmp/draft.gguf",
                        ...
                        /*vocab=*/64));
    CHECK(dw.odistill == nullptr);
}

TEST_CASE(ODistillUnitFixture, resolved_semantics_match_python_golden) {
    const DraftWeights dw = tiny_drafter();
    CHECK_EQUAL(odistill_resolved_drafter_semantics(dw), test_semantics());
    CHECK_EQUAL(odistill_semantics_tag(test_semantics()),
                std::string("1ec7f022739c0547"));
    CHECK_EQUAL(odistill_semantics_tag(test_target_sha() + ";" + test_semantics()),
                std::string("f234d12ce461a038"));
    const std::string contract = odistill_adapter_contract(
    ...
    CHECK_EQUAL(odistill_semantics_tag(contract),
                std::string("d7562c1f882d7970"));
    CHECK(odistill_adapter_contract(test_sha(), test_target_sha(), test_semantics(),
                                  8, 32.0f) != contract);
    CHECK(odistill_adapter_contract(test_sha(), test_target_sha(), test_semantics(),
                                  4, 16.0f) != contract);
    CHECK(odistill_adapter_contract(std::string(64, 'b'), test_target_sha(),
...
struct RingView {
    ...
    void set_tail(uint64_t v) {
        f.seekp((std::streamoff)offsetof(ODistillRingHeader, tail));
...
TEST_CASE(ODistillUnitFixture, ring_create_publishes_stream_facts) {
    ODistillRing ring;
    const std::string name =
        "/odistill-test-" + std::to_string((long)getpid()) + "-a";
    REQUIRE(ring.create(name, 1 << 20, 0x1122334455667788ull,
                        0x8877665544332211ull, 0x1ec7f022739c0547ull,
                        5, 5120, 16, 32, 248320));
    RingView v(ring);
    CHECK_EQUAL(v.hdr->magic, ODISTILL_RING_MAGIC);
    CHECK_EQUAL(v.hdr->version, ODISTILL_RING_VERSION);
...
TEST_CASE(ODistillUnitFixture, ring_create_removes_dead_engine_segment) {
    std::string stale_name;
    const pid_t child = ::fork();
    REQUIRE(child >= 0);
    if (child == 0) {
        stale_name = "/lucebox-odistill-" +
    ...
    stale_name = "/lucebox-odistill-" + std::to_string((long)child);
    const std::string stale_path = "/dev/shm" + stale_name;
    REQUIRE(::access(stale_path.c_str(), F_OK) == 0);

    ODistillRing ring;
    const std::string name =
        "/odistill-test-" + std::to_string((long)getpid()) + "-cleanup";
    REQUIRE(ring.create(name, 1 << 20, 1, 2, 3, 1, 4, 2, 0, 100));
    CHECK(::access(stale_path.c_str(), F_OK) != 0);
    ring.close();
}

TEST_CASE(ODistillUnitFixture, ring_push_roundtrips_records) {
    ODistillRing ring;
    const std::string name =
        "/odistill-test-" + std::to_string((long)getpid()) + "-b";
    REQUIRE(ring.create(name, 1 << 20, 1, 2, 3, 1, 4, 2, 0, 100));

    ODistillRecordHeader h{};
    h.type = ODISTILL_REC_STEP;
    h.seq_id = 7;
    h.pos = 42;
    h.n_rows = 2;
    ...
    ODistillRecordHeader got{};
    std::memcpy(&got, rec.data(), sizeof(got));
    CHECK_EQUAL(got.type, (uint32_t)ODISTILL_REC_STEP);
...
TEST_CASE(ODistillUnitFixture, ring_drops_when_full_and_pads_at_wrap) {
    ODistillRing ring;
    const std::string name =
        "/odistill-test-" + std::to_string((long)getpid()) + "-c";
    // Tiny ring: 1 MiB min is enforced, so use 1 MiB and a big record.
    REQUIRE(ring.create(name, 1 << 20, 1, 2, 3, 1, 4, 2, 0, 100));

    std::vector<uint8_t> big((1 << 18) - 32);  // record = 256 KiB exactly
    ODistillRecordHeader h{};
    h.type = ODISTILL_REC_CONTEXT;
...
TEST_CASE(ODistillUnitFixture, ring_pad_record_written_at_wrap) {
    ODistillRing ring;
    const std::string name =
        "/odistill-test-" + std::to_string((long)getpid()) + "-d";
    ...
    auto pad = v.read_at((3 << 19) + (1 << 20), sizeof(ODistillRecordHeader));
    ODistillRecordHeader ph{};
    std::memcpy(&ph, pad.data(), sizeof(ph));
    CHECK_EQUAL(ph.type, (uint32_t)ODISTILL_REC_PAD);
    CHECK_EQUAL(ph.size_bytes, (uint32_t)(3 << 19));
    ring.close();
}

// Golden bytes shared with optimizations/odistill tests: one STEP record with
// known contents must serialize to exactly these bytes (little-endian).
TEST_CASE(ODistillUnitFixture, ring_step_record_golden_bytes) {
    ODistillRing ring;
    const std::string name =
        "/odistill-test-" + std::to_string((long)getpid()) + "-e";
...
TEST_CASE(ODistillUnitFixture, guard_promotes_on_equal_al) {
    ODistillGuardConfig cfg;
    cfg.probation_steps = 4;
    cfg.min_baseline_steps = 2;
    cfg.min_steps_between_swaps = 8;
    ODistillGuard g(cfg);
    for (int i = 0; i < 8; i++) {
        CHECK(g.record_step(5.0f) == ODistillGuardAction::None);
    }
    CHECK(g.can_swap());
    g.on_swap(1);
    CHECK(g.state() == ODistillGuard::State::Probation);
    CHECK(!g.can_swap());
    ODistillGuardAction a = ODistillGuardAction::None;
    for (int i = 0; i < 4; i++) a = g.record_step(5.0f);
    CHECK(a == ODistillGuardAction::Promote);
    CHECK(g.state() == ODistillGuard::State::Serving);
    CHECK_EQUAL(g.promotes(), (uint64_t)1);
    CHECK_EQUAL(g.current_generation(), (uint64_t)1);
}

TEST_CASE(ODistillUnitFixture, guard_retains_promotion_decision_operands) {
    ODistillGuardConfig cfg;
    cfg.probation_steps = 4;
    cfg.min_baseline_steps = 2;
    ...
    g.on_swap(1);
    ODistillGuardAction action = ODistillGuardAction::None;
    for (int i = 0; i < 4; ++i) action = g.record_step(4.9f);
    CHECK(action == ODistillGuardAction::Promote);
...
TEST_CASE(ODistillUnitFixture, guard_rolls_back_on_regression_and_backs_off) {
    ODistillGuardConfig cfg;
    cfg.probation_steps = 4;
    cfg.min_baseline_steps = 2;
    cfg.min_steps_between_swaps = 4;
    ...
    CHECK(a == ODistillGuardAction::Rollback);
...
TEST_CASE(ODistillUnitFixture, guard_disables_after_consecutive_rollbacks) {
    ODistillGuardConfig cfg;
    ...
    ODistillGuardAction last = ODistillGuardAction::None;
    for (int r = 0; r < 3; r++) {
    ...
    CHECK(last == ODistillGuardAction::Disable);
    CHECK(g.state() == ODistillGuard::State::Disabled);
    // Disabled guard ignores further samples and refuses swaps.
    CHECK(g.record_step(9.0f) == ODistillGuardAction::None);
    CHECK(!g.can_swap());
}

TEST_CASE(ODistillUnitFixture, guard_auto_promotes_without_baseline) {
    ODistillGuardConfig cfg;
    cfg.probation_steps = 2;
...
TEST_CASE(ODistillUnitFixture, adapter_expected_tensor_specs) {
    DraftWeights dw = tiny_drafter();
    const auto specs = odistill_lora_expected_tensors(dw, 4);
...
TEST_CASE(ODistillUnitFixture, adapter_load_accepts_valid_file) {
    DraftWeights dw = tiny_drafter();
    const std::string path =
        "/tmp/odistill-test-adapter-" + std::to_string((long)getpid()) + ".st";
    write_adapter_file(path, dw, 4, test_sha(), 3);
    ODistillAdapterHost host;
    std::string err;
    REQUIRE(odistill_adapter_load(path, dw, 4, 32.0f, test_sha(),
...
TEST_CASE(ODistillUnitFixture, adapter_load_refuses_non_finite_tensor_values) {
    DraftWeights dw = tiny_drafter();
    const std::string path =
        "/tmp/odistill-test-adapter-" + std::to_string((long)getpid()) + "n.st";
    ODistillAdapterHost host;
    std::string err;
    ...
    CHECK(!odistill_adapter_load(path, dw, 4, 32.0f, test_sha(),
                               test_target_sha(), test_semantics(),
                               "default", host, err));
...
TEST_CASE(ODistillUnitFixture, adapter_load_refuses_mismatches) {
    DraftWeights dw = tiny_drafter();
    const std::string path =
        "/tmp/odistill-test-adapter-" + std::to_string((long)getpid()) + "r.st";
    ODistillAdapterHost host;
    std::string err;
    ...
    // Format v3 requires the complete digest, never a convenient prefix.
    ...
    // Wrong rank.
    write_adapter_file(path, dw, 4, test_sha(), 1);
    CHECK(!odistill_adapter_load(path, dw, 8, 32.0f, test_sha(),
                               test_target_sha(), test_semantics(),
                               "default", host, err));
...
TEST_CASE(ODistillUnitFixture, adapter_load_refuses_malformed_field_types) {
    const std::string path =
        "/tmp/odistill-test-adapter-" + std::to_string((long)getpid()) + "j.st";
    nlohmann::json header;
    header["bad"] = {
        {"dtype", 7},
        {"shape", "not-an-array"},
...
#if !defined(_WIN32)
TEST_CASE(ODistillUnitFixture, supervisor_spawns_and_relays_swap_events) {
    ...
    const std::string script =
        "/tmp/odistill-test-trainer-" + std::to_string((long)getpid()) + ".sh";
    {
    ...
    ODistillSupervisor sup;
    ODistillSupervisorConfig cfg;
...
TEST_CASE(ODistillUnitFixture, supervisor_rollback_barrier_and_disabled_state) {
...
TEST_CASE(ODistillUnitFixture, supervisor_survives_missing_binary) {
    ODistillSupervisor sup;
    ODistillSupervisorConfig cfg;
    cfg.trainer_bin = "/nonexistent/odistill-trainer";
...
TEST_CASE(ODistillUnitFixture, supervisor_closes_unrelated_fds_before_exec) {
    const std::string probe_path =
        "/tmp/odistill-fd-probe-" + std::to_string((long)getpid());
    const int probe_fd = ::open(probe_path.c_str(), O_CREAT | O_RDWR, 0600);
    REQUIRE(probe_fd >= 3);

    const std::string script =
        "/tmp/odistill-fd-trainer-" + std::to_string((long)getpid()) + ".sh";
    {
        std::ofstream f(script);
        f << "#!/bin/bash\n"
             "stream=2\nprobe=-1\n"
...
#if !defined(_WIN32)
TEST_CASE(ODistillUnitFixture, store_promoted_roundtrip) {
    const std::string dir =
        "/tmp/odistill-test-store-" + std::to_string((long)getpid());
    ::mkdir(dir.c_str(), 0700);
    CHECK(odistill_store_write_promoted(dir, dir + "/gen3.safetensors", 3));
    std::string path;
    uint64_t gen = 0;
    REQUIRE(odistill_store_read_promoted(dir, path, gen));
...
TEST_CASE(ODistillUnitFixture, store_promoted_rejects_malformed_types) {
    const std::string dir =
        "/tmp/odistill-test-store-bad-" + std::to_string((long)getpid());
    ::mkdir(dir.c_str(), 0700);
    {
    ...
    std::string path;
    uint64_t gen = 0;
    CHECK(!odistill_store_read_promoted(dir, path, gen));
    std::remove((dir + "/promoted.json").c_str());
    ::rmdir(dir.c_str());
}
#endif
