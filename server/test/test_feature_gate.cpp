...
static void test_feature_gate_gbatching_is_narrow_and_fail_closed() {
    BackendArgs args;
    args.model_path = "/nonexistent/model.gguf";
    args.gbatching_mode = true;
    ...
    args.gbatching_branches = 8;
    args.gbatching_horizon = 8; // root + 64 candidates exceeds the MVP cap
    TEST_ASSERT(!gate_result(args, "qwen35", PlacementBackend::Cuda).empty());
}
...
int main() {
    ...
    RUN_TEST(test_feature_gate_gbatching_is_narrow_and_fail_closed);
