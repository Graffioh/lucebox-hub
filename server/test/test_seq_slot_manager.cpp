...
int main() {
    // 8 blocks x 16 tokens = 128 pool tokens, 2 slots, per-seq max_ctx 64.
    {
        ...
        CHECK(!mgr.admit(1, 0, greedy_sampler()).ok);
        CHECK(!mgr.admit(1, 65, greedy_sampler()).ok);
        CHECK(!mgr.is_active(0) && !mgr.is_active(1));
        ...
        auto a = mgr.admit(1, /*prompt_len=*/20, greedy_sampler());
        CHECK(a.ok && !a.busy);
        CHECK(a.slot == 0);
        ...
        auto b = mgr.admit(2, 20, greedy_sampler());
        CHECK(b.ok && b.slot == 1);
        ...
        auto c = mgr.admit(3, 8, greedy_sampler());
        CHECK(!c.ok && c.busy);
        ...
        auto d = mgr.admit(3, 8, greedy_sampler());
        CHECK(d.ok && d.slot == 0);
    ...
    // prompt impossible: this one-block pool falls back to prompt-only.
    {
        ...
        auto a = mgr.admit(1, /*prompt_len=*/8, greedy_sampler());
        CHECK(a.ok);
    ...
    // Busy-vs-never-fits classification against a small pool.
    {
        ...
        auto exact = mgr.admit(1, 60, greedy_sampler());
        ...
        auto big = mgr.admit(2, 48, greedy_sampler());  // 3 prompt + 1 headroom
        CHECK(big.ok);
        CHECK(mgr.append_prefill(big.slot, 48).ok);
        auto blocked = mgr.admit(3, 32, greedy_sampler());  // 2 blocks
        CHECK(!blocked.ok && blocked.busy);
        mgr.retire(big.slot);
        auto now_fits = mgr.admit(3, 32, greedy_sampler());
        CHECK(now_fits.ok);
    }
    ...
    // busy — waiting for a drain could never help.
    {
        ...
        PagedKvPool pool(4, 2, /*block_size=*/16);
        SeqSlotManager mgr(pool, /*max_ctx=*/128);
        auto never = mgr.admit(1, 100, greedy_sampler());
        ...
        auto live = mgr.admit(2, 16, greedy_sampler());
        CHECK(live.ok);
        auto live2 = mgr.admit(3, 16, greedy_sampler());
        CHECK(live2.ok);
        auto still_never = mgr.admit(4, 100, greedy_sampler());
    ...
    // consuming the same capacity and reaching a no-progress deadlock.
    {
        ...
        SeqSlotManager mgr(pool, /*max_ctx=*/64);
        auto a = mgr.admit(1, 48, greedy_sampler());
        CHECK(a.ok);
        CHECK(pool.free_block_count() == 0);
        ...
        auto later = mgr.admit(2, 32, greedy_sampler());
        ...
        later = mgr.admit(2, 32, greedy_sampler());
        CHECK(!later.ok && later.busy);
    ...
    // later are topped up for that decoder before a new prompt can claim them.
    {
        ...
        auto older = mgr.admit(1, /*prompt_len=*/16, greedy_sampler());
        auto peer = mgr.admit(2, /*prompt_len=*/16, greedy_sampler());
        ...
        auto younger = mgr.admit(3, /*prompt_len=*/16, greedy_sampler());
        ...
        younger = mgr.admit(3, /*prompt_len=*/16, greedy_sampler());
        CHECK(younger.ok);
    }
    ...
    // admission-wide operation, never a side effect of append_token().
    {
        ...
        auto first = mgr.admit(1, /*prompt_len=*/4, greedy_sampler());
        auto boundary = mgr.admit(2, /*prompt_len=*/4, greedy_sampler());
    ...
    // until commit_prefill() makes its first sampled token available.
    {
        ...
        SeqSlotManager mgr(pool, 64);
        auto a = mgr.admit(1, 20, greedy_sampler());
        CHECK(a.ok);
    ...
    // and be given argmax (or the reverse).
    {
        ...
        auto a = mgr.admit(1, 4, cfg);
        CHECK(a.ok);
        const uint64_t first = mgr.slot(0).rng();
        mgr.retire(0);
        auto b = mgr.admit(2, 4, cfg);
        CHECK(b.ok);
