...
// prompt-only admission checks, on-demand prefill/decode
// allocation, block-table deltas, exhaustion atomicity, and the
// pool-handle lifecycle across retire.
...
int main() {
    // 8 blocks x 16 tokens = 128 pool tokens, 2 slots, per-seq max_ctx 64.
    {
        ...
        // Admission claims identity only; it does not allocate the prompt.
    ...
    // Admission does not reserve speculative output space: a prompt that fits
    // the current pool claims a slot and leaves allocation to append_prefill.
    {
    ...
    // Busy-vs-never-fits classification against a small pool.
    {
        ...
        // A near-limit prompt is admitted without reserving output blocks;
        // the scheduler owns the generation cap via max_context().
        ...
        auto big = mgr.admit(2, 48, greedy_sampler());  // 3 blocks
        CHECK(big.ok);
    ...
    // block table changes when a whole chunk cannot be allocated.
    {
        ...
        // not append penalty history or advance the position.
