...
int main() {
    // 8 blocks x 16 tokens = 128 pool tokens, 2 slots, per-seq max_ctx 64.
    {
        ...
        CHECK(!mgr.admit(1, prompt_tokens(0), greedy_sampler()).ok);
        CHECK(!mgr.admit(1, prompt_tokens(65), greedy_sampler()).ok);
        CHECK(!mgr.is_active(0) && !mgr.is_active(1));
        ...
        auto a = mgr.admit(1, admitted_prompt, greedy_sampler());
        CHECK(a.ok && !a.busy);
        CHECK(a.slot == 0);
        ...
        auto b = mgr.admit(2, prompt_tokens(20), greedy_sampler());
        CHECK(b.ok && b.slot == 1);
        ...
        auto c = mgr.admit(3, prompt_tokens(8), greedy_sampler());
        CHECK(!c.ok && c.busy);
        ...
        auto d = mgr.admit(3, prompt_tokens(8), greedy_sampler());
        CHECK(d.ok && d.slot == 0);
    ...
    // prompt impossible: this one-block pool falls back to prompt-only.
    {
        ...
        auto a = mgr.admit(1, prompt_tokens(8), greedy_sampler());
        CHECK(a.ok);
    ...
    // Busy-vs-never-fits classification against a small pool.
    {
        ...
        auto exact = mgr.admit(1, prompt_tokens(60), greedy_sampler());
        CHECK(exact.ok && mgr.max_context() == 64);
        CHECK(pool.free_block_count() == 0);
        ...
        auto big = mgr.admit(2, prompt_tokens(48), greedy_sampler());  // 3 prompt + 1 headroom
        CHECK(big.ok);
        CHECK(mgr.append_prefill(big.slot, 48).ok);
        auto blocked = mgr.admit(3, prompt_tokens(32), greedy_sampler());  // 2 blocks
        CHECK(!blocked.ok && blocked.busy);
        mgr.retire(big.slot);
        auto now_fits = mgr.admit(3, prompt_tokens(32), greedy_sampler());
        CHECK(now_fits.ok);
    }
    ...
    // busy — waiting for a drain could never help.
    {
        ...
        PagedKvPool pool(4, 2, /*block_size=*/16);
        SeqSlotManager mgr(pool, /*max_ctx=*/128);
        auto never = mgr.admit(1, prompt_tokens(100), greedy_sampler());
        ...
        auto live = mgr.admit(2, prompt_tokens(16), greedy_sampler());
        CHECK(live.ok);
        auto live2 = mgr.admit(3, prompt_tokens(16), greedy_sampler());
        CHECK(live2.ok);
        auto still_never = mgr.admit(4, prompt_tokens(100), greedy_sampler());
    ...
    // consuming the same capacity and reaching a no-progress deadlock.
    {
        ...
        SeqSlotManager mgr(pool, /*max_ctx=*/64);
        auto a = mgr.admit(1, prompt_tokens(48), greedy_sampler());
        CHECK(a.ok);
        CHECK(pool.free_block_count() == 0);
        ...
        later = mgr.admit(2, prompt_tokens(32), greedy_sampler());
        CHECK(!later.ok && later.busy);
        auto too_much = mgr.append_prefill(a.slot, 33);
        CHECK(!too_much.ok && !too_much.busy);
        auto final_chunk = mgr.append_prefill(a.slot, 32);
        CHECK(final_chunk.ok && !final_chunk.busy);
        mgr.commit_prefill(a.slot, 48);
    ...
    // page, admission falls back to prompt-only and exhaustion stays retryable.
    {
        ...
        SeqSlotManager mgr(pool, /*max_ctx=*/32);
        auto a = mgr.admit(1, prompt_tokens(16), greedy_sampler());
        CHECK(a.ok);
    ...
    // later are topped up for that decoder before a new prompt can claim them.
    {
        PagedKvPool pool(/*physical_block_count=*/4,
                         /*max_sequences=*/2, /*block_size=*/16);
        SeqSlotManager mgr(pool, /*max_ctx=*/48);
        auto older = mgr.admit(1, prompt_tokens(16), greedy_sampler());
        auto peer = mgr.admit(2, prompt_tokens(16), greedy_sampler());
        CHECK(older.ok && peer.ok);
        CHECK(pool.free_block_count() == 0);
        CHECK(mgr.append_prefill(older.slot, 16).ok);
        CHECK(mgr.append_prefill(peer.slot, 16).ok);
        mgr.commit_prefill(older.slot, 16);
        mgr.commit_prefill(peer.slot, 16);
        ...
        auto younger = mgr.admit(3, prompt_tokens(16), greedy_sampler());
        CHECK(!younger.ok && younger.busy);
        CHECK(pool.sequence(mgr.slot(older.slot).handle, snapshot) ==
              PagedKvStatus::Ok);
        ...
        younger = mgr.admit(3, prompt_tokens(16), greedy_sampler());
        CHECK(younger.ok);
    }
    ...
    // admission-wide operation, never a side effect of append_token().
    {
        ...
        SeqSlotManager mgr(pool, /*max_ctx=*/16);
        auto first = mgr.admit(1, prompt_tokens(4), greedy_sampler());
        auto boundary = mgr.admit(2, prompt_tokens(4), greedy_sampler());
        CHECK(first.ok && boundary.ok);
        ...
        mgr.commit_prefill(first.slot, 4);
        mgr.commit_prefill(boundary.slot, 4);
    ...
    // until commit_prefill() makes its first sampled token available.
    {
        ...
        SeqSlotManager mgr(pool, 64);
        auto a = mgr.admit(1, prompt_tokens(20), greedy_sampler());
        CHECK(a.ok);
    ...
    // and be given argmax (or the reverse).
    {
        ...
        auto a = mgr.admit(1, prompt_tokens(4), cfg);
        CHECK(a.ok);
        const uint64_t first = mgr.slot(0).rng();
        mgr.retire(0);
        auto b = mgr.admit(2, prompt_tokens(4), cfg);
        CHECK(b.ok);
        ...
        mgr.retire(0);
        cfg.seed = 5678;
        auto c = mgr.admit(3, prompt_tokens(4), cfg);
        CHECK(c.ok);
        CHECK(mgr.slot(0).rng() != first);
    }
