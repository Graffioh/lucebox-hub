...
int main() {
    ...
    CHECK(resolve_chain_verify_depth(0, 4) == 4);
    CHECK(resolve_chain_verify_depth(2, 4) == 2);
    CHECK(resolve_chain_verify_depth(4, 4) == 4);
    CHECK(resolve_chain_verify_depth(1, 4) == 0);
    CHECK(resolve_chain_verify_depth(5, 4) == 0);
    CHECK(resolve_chain_verify_depth(0, 1) == 0);
    std::vector<int32_t> short_draft = draft;
    CHECK(truncate_chain_proposal(short_draft, 2));
    CHECK((short_draft == std::vector<int32_t>{10, 11}));
    const DDTree short_tree = make_chain_verify_tree(short_draft);
    CHECK(short_tree.n_nodes == 1);
    CHECK((short_tree.parents == std::vector<int>{-1, 0}));
    const std::vector<int32_t> before_invalid = short_draft;
    CHECK(!truncate_chain_proposal(short_draft, 1));
    CHECK(short_draft == before_invalid);
    CHECK(!truncate_chain_proposal(short_draft, 3));
    CHECK(short_draft == before_invalid);
    ...
    CHECK(ar_after_spec_failures.ar_bucket == 2);
    CHECK(ar_after_spec_failures.commit_rows == 2);

    CHECK(chain_lane_disposition(false, false) ==
          ChainLaneDisposition::AR);
    CHECK(chain_lane_disposition(true, false) ==
          ChainLaneDisposition::Speculation);
    CHECK(chain_lane_disposition(true, true) ==
          ChainLaneDisposition::Failed);
    CHECK(chain_lane_disposition(false, true) ==
          ChainLaneDisposition::Failed);
    CHECK(chain_lane_executes(ChainLaneDisposition::AR));
    CHECK(chain_lane_executes(ChainLaneDisposition::Speculation));
    CHECK(!chain_lane_executes(ChainLaneDisposition::Failed));
