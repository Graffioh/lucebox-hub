...
static void test_compact_append() {
    PagedKvPool pool(8, 1);
    const auto handle = acquire(pool, 123);

    const auto prompt = pool.append_compact(handle, 33);
    ...
    const auto token = pool.append_compact(handle, 1);
    ...
    const auto empty = pool.append_compact(handle, 0);
...
static void test_auto_pool_sizing_and_watermark() {
...
int main() {
    ...
    test_auto_pool_sizing_and_watermark();
