...
bool run_case(
        ggml_backend_t backend, bool kda, bool raw_gates,
        bool report_timing) {
    ...
    for (int prefix = 0; ok && prefix <= T; ++prefix) {
        ...
        ok = ggml_backend_cuda_gdn_transition_journal_commit(
    ...
    if (ok) {
        const std::vector<int32_t> out_of_range_slots{2, 99, 0, 1};
        upload_f32(tensors.mapped_state, mapped_base);
        ...
        if (ok) {
            ggml_backend_tensor_get(
                ...
                mapped_actual.size()*sizeof(float));
            ok = compare_vectors(
                "out-of-range padded slot", mapped_actual, mapped_expected,
                STATE_TOLERANCE);
        }
    }
    ...
    if (ok) {
        ...
        ok = ok && !ggml_backend_cuda_gdn_transition_journal_commit(
            tensors.journal, tensors.identity_state,
            tensors.accepted, tensors.slots);
        ggml_backend_tensor_get(
            tensors.identity_state, actual_state.data(), 0,
    ...
    if (ok && report_timing) {
        ...
        elapsed_us.reserve(repetitions);
        accepted.assign(B, T);
        ggml_backend_tensor_set(tensors.accepted, accepted.data(), 0,
                                accepted.size()*sizeof(accepted[0]));
        ggml_backend_tensor_set(tensors.slots, identity_slots.data(), 0,
        ...
        for (int repetition = 0; repetition < repetitions; ++repetition) {
            ...
            const bool committed =
                ggml_backend_cuda_gdn_transition_journal_commit(
                    tensors.journal, tensors.identity_state,
                    tensors.accepted, tensors.slots);
        ...
        if (ok) {
            ...
            std::printf(
                "gdn journal commit S=%d H=%d T=%d B=%d median %.1f us (synchronous Phase 1)\n",
                S, H, T, B, elapsed_us[elapsed_us.size()/2]);
        }
    }
...
bool run_grouped_tree_case(ggml_backend_t backend) {
    ...
    ggml_backend_tensor_set(
        ...
        identity_slots.size()*sizeof(identity_slots[0]));
    ok = ok && ggml_backend_cuda_gdn_transition_journal_commit(
        journal, committed_state, accepted, slots);
...
int main() {
    ...
    bool ok = run_case(backend, false, false, true);
