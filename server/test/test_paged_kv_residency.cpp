// Pure-host tests for multi-sequence paged K/V residency. Transfers use a
// deterministic mock device array but preserve async queue/barrier semantics.
...
TEST_CASE(PagedKvResidencyFixture,
    ...
    // Teardown retries the quarantined barrier instead of being rejected by
    // the manager-wide failure latch.
    io.fail_sync = false;
...
TEST_CASE(PagedKvResidencyFixture,
    ...
    // The valid prefix remains evictable, allowing the peer to make progress.
...
TEST_CASE(PagedKvResidencyFixture,
    ...
    // Simulate one synchronized packed target graph, then make room. The still
    // pending peer page cannot be the victim even though another slot grows.
    CHECK(pager.commit_pending_writes(first) == PagedKvResidencyStatus::Ok);
...
TEST_CASE(PagedKvResidencyFixture,
    ...
    // No operation may reuse or mutate the quarantined destination while the
    // failed stream is not known to be drained.
    CHECK(pager.evict_block(handle, 0, /*allow_protected=*/true) ==
          PagedKvResidencyStatus::TransferFailed);
