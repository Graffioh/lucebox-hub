...
int main(int argc, char ** argv) {
    ...
    const std::vector<int32_t> active_slot_ids{
        physical_n_seq - 1, physical_n_seq - 2, -1, 4,
    };
    ...
    // multi-partition full prefix, a block-aligned prefix, INT32_MAX, and
    // a padding row, interleaved across the two block-table columns.
    // Asymmetric
    // quantized K/V pins both dequant paths under the causal clamp.
    const std::vector<int32_t> ragged_slot_ids{1, 0, 1, 0, 0, -1};
    const std::vector<int32_t> ragged_positions{
