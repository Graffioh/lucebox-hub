...
bool HttpServer::deliver_generation_token(
        ...
        bool & client_disconnected, ClientSendBuffer * send_buffer) {
    if (client_disconnected) return false;
    ++completion_tokens;

    auto put = [&](const std::string & chunk) {
        if (send_buffer) {
    ...
    const bool publish_request_status = send_buffer == nullptr;
    if (publish_request_status && completion_tokens % 10 == 0) {
    ...
    if (!text.empty()) {
        ...
        if (publish_request_status) broadcast_token(text);
    }
...
void HttpServer::send_nonstream_response(
    ...
    case ApiFormat::ANTHROPIC:
        ...
        break;
    case ApiFormat::RESPONSES:
        response = build_responses_api_response(
            req, result, gen_timings, counts, emitter);
        break;
    default:
        response = {{"text", emitter.accumulated_text()}};
        break;
    }
