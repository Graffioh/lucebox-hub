...
void HttpServer::scheduler_loop(SeqEngine & engine) {
    ...
    auto advance_slot = [&](SchedSlot & s, int32_t tok) {
        ...
        // Heartbeats are needed while prefill is silent. Once token frames enter
        // the buffered writer, stop them so a comment cannot split a partial
        // SSE frame across separate non-blocking flushes.
        if (s.job->req.stream) stop_job_stream(s.job);
