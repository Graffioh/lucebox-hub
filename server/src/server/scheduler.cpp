...
// classic one-job-at-a-time worker_loop stays in http_server.cpp; the two
// share deliver_generation_token / send_nonstream_response so both paths
// emit byte-identical wire formats.
...
void HttpServer::scheduler_loop(SeqEngine & engine) {
    ...
    auto publish_live_count = [&]() {
        ...
        status_.set_concurrent_requests(n);
        broadcast_status();
    };
    ...
    // slot — on EOS, gen cap, client gone, or degenerate repetition.
    auto advance_slot = [&](SchedSlot & s, int32_t tok) {
        ...
        const bool cont = deliver_generation_token(
            s.job->req, s.fd, *s.emitter, tok, s.completion_tokens,
            s.visible_output_seen, s.client_disconnected, &s.send_buffer);
        s.pending_tok = tok;
    ...
    auto retire_slot = [&](int idx, bool backend_ok) {
        ...
        if (s.failed || !backend_ok) {
            if (!s.client_disconnected) {
                if (req.stream) {
                    json err = {{"error", {{"message", "generation failed"},
                                           {"type", "server_error"}}}};
                    s.send_buffer.append("data: " + err.dump() + "\n\n");
                    s.send_buffer.append("data: [DONE]\n\n");
                } else {
                    json err = {{"error", {{"message", "generation failed"},
            ...
            send_nonstream_response(req, s.fd, *s.emitter, s.gen_tokens,
                                    ...
                                    s.degenerate_close, /*accept_rate=*/0.0f,
                                    gen_timings, &s.send_buffer);
        }
    ...
    auto admit_job = [&](ServerJob * job) -> AdmitOutcome {
        ...
        // The prefill runs inside admit(); decode of other slots pauses.
        ...
        s.prefill_s = ar.prefill_s;
        s.n_gen_cap = n_gen_cap;
        ...
        // forward handoff as the classic AR loop's first token. Flush it
        // right away: it is the client's time-to-first-token.
        advance_slot(s, ar.first_token);
    ...
    while (true) {
        ...
        // or lets the buffer hit the cap is dropped (its slot retires).
        {
            ...
            for (int i = 0; i < n_slots; i++) {
                ...
                if (s.send_buffer.should_drop(now, std::chrono::seconds(30),
                                              kMaxSlotSendBuffer)) {
                    std::fprintf(stderr,
