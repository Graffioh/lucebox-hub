...
bool build_target_step_paged_tree(
    ...
    int kq_stride_pad,
    int mapped_ar_seqs,
    bool capture_tree_commit) {
    (void)kq_stride_pad;
    step_graph_free(sg);

    if (mapped_ar_seqs < 0 ||
        (mapped_ar_seqs > 0 && !capture_tree_commit) ||
        mapped_ar_seqs + n_tree_seqs > cache.n_seq_slots ||
        mapped_ar_seqs + n_tree_seqs > 64) {
        return false;
    }
    ...
    gi.n_tokens = n_tokens;
    gi.kv_start = 0;
    gi.capture_layers = capture_tree_commit;
    gi.capture_delta_intermediate = false;
    gi.capture_tree_commit = capture_tree_commit;
    gi.parent_ids = sg.parent_ids;
    ...
    if (capture_tree_commit &&
        (!sg.tree_features || sg.delta_captures.empty())) {
        return false;
    }
    ggml_set_output(sg.logits);
    sg.argmax_tokens = ggml_argmax(sg.ctx, sg.logits);
    ...
    if (!capture_tree_commit) return true;
