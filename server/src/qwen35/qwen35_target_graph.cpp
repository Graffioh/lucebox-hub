...
bool create_target_cache_partial(const TargetWeights & w,
    ...
    // may grow it to the next whole block; other oversized overrides preserve
    // the legacy max_ctx fallback.
    const bool bounded_pool = ctx_alloc > 0 && ctx_alloc < max_ctx;
    const bool paged_padding =
        paged_attention && ctx_alloc == paged_token_capacity(max_ctx);
    ...
    const int ctx_phys =
        (bounded_pool || paged_padding || (multi_slot && ctx_alloc > 0))
            ? ctx_alloc : max_ctx;
...
static ggml_tensor * build_full_attn_block(
    ...
    if (paged_block_table) {
        ...
        // the launch bound must cover the longest live slot instead.
        const int launch_kv_len = paged_max_kv_len > 0 ? paged_max_kv_len : kv_len;
        int paged_launch_len =
            ((launch_kv_len + fattn_stride - 1) / fattn_stride) * fattn_stride;
        ...
        attn = paged_query_seq_ids
            ? ggml_paged_attn_ext(ctx, Qfa, cache_k, cache_v,
                                  ...
                                  paged_launch_len)
            : active_slot_ids
                ? ggml_paged_attn_active(ctx, Qfa, cache_k, cache_v,
                                         paged_block_table,
                                         paged_kv_seq_lens,
                                         active_slot_ids, kq_scale,
                                         PAGED_BLOCK_SIZE, paged_launch_len)
                : ggml_paged_attn(ctx, Qfa, cache_k, cache_v,
                                  paged_block_table, paged_kv_seq_lens,
                                  kq_scale, PAGED_BLOCK_SIZE,
                                  paged_launch_len);
        if (n_tokens > 1) {
        ...
        ggml_tensor * read_k = cache_k;
        ggml_tensor * read_v = cache_v;
        int win_len_padded =
            ((win_len + fattn_stride - 1) / fattn_stride) * fattn_stride;
        if (step_invariant) {
...
// True when the selective write list names every row of the segment exactly
// once. Then a single whole-batch copy is semantically identical to one copy
// per slot. At 16 live rows the per-slot form emits 30 extra copies per
// DeltaNet layer (two state writes times 15 additional copies), or 1440 extra
// copy kernels over Qwen3.6's 48 DeltaNet layers.
static bool delta_net_writes_every_row(const int * slots, int n, int seqs) {
    if (!slots || n != seqs || seqs <= 0) return false;
    std::vector<bool> seen((size_t)seqs, false);
    for (int i = 0; i < n; ++i) {
        const int s = slots[i];
        if (s < 0 || s >= seqs || seen[(size_t)s]) return false;
        seen[(size_t)s] = true;
    }
    return true;
}
...
static ggml_tensor * build_delta_net_block(
    ...
    // Batched decode: n_seqs independent recurrences of n_tokens/n_seqs
    // timesteps each. Token t of the flat token axis belongs to sequence
    // t / n_seq_tokens; the passed state tensors carry the matching batch
    // axis. Both ggml_ssm_conv and ggml_gated_delta_net iterate sequences
    // independently, so this is a pure reshape at the graph level.
    int n_seqs = 1,
    const QwenPrefillSegment * prefill_segments = nullptr,
    int n_prefill_segments = 0,
    int decode_token_offset = -1,
    const int * state_write_slots = nullptr,
    int n_state_write_slots = 0,
    ggml_tensor * active_slot_ids = nullptr,
    ...
    // A paged step is segmented even when it contains decode rows only. In
    // that case the selective write list still matters: inactive rows must
    // not overwrite recurrent state that a later admission will reuse.
    const bool ragged = n_prefill_segments > 0 || decode_token_offset >= 0;
    GGML_ASSERT(!ragged || n_seqs >= 1);
    GGML_ASSERT(n_prefill_segments == 0 || prefill_segments);
    GGML_ASSERT(ragged || (n_seqs >= 1 && n_tokens % n_seqs == 0));
    GGML_ASSERT((n_seqs == 1 && !ragged) || (!cap && !parent_ids));
    GGML_ASSERT((active_slot_ids == nullptr) == (state_slot_ids == nullptr));
    GGML_ASSERT(!active_slot_ids ||
                (!cap && !parent_ids && decode_token_offset >= 0));
    ...
    struct DeltaSeg {
        ...
        ggml_tensor * conv_st;
        ggml_tensor * ssm_st;
        const int * write_slots;  // null = persist every sequence
        int n_write_slots;
    };
    ...
    if (ragged) {
        ...
        for (int i = 0; i < n_prefill_segments; ++i) {
            ...
            GGML_ASSERT(pf.token_offset == end && pf.n_tokens > 0 &&
                        pf.seq_slot >= 0 &&
                        pf.seq_slot < conv_state->ne[2]);
            ...
            segs.push_back({pf.token_offset, pf.n_tokens, 1, c, s,
                            nullptr, 0});
            end += pf.n_tokens;
        }
        if (decode_token_offset >= 0) {
            GGML_ASSERT(decode_token_offset == end &&
                        decode_token_offset + n_seqs == n_tokens);
            segs.push_back({decode_token_offset, 1, n_seqs,
                            conv_state, ssm_state,
                            state_write_slots, n_state_write_slots});
            end += n_seqs;
        }
        GGML_ASSERT(end == n_tokens);
    } else {
        segs.push_back({0, n_tokens / n_seqs, n_seqs,
                        conv_state, ssm_state, nullptr, 0});
    }
    ...
    std::vector<ggml_tensor *> flat(segs.size(), nullptr);
    ...
    const bool seg_active = active_slot_ids &&
        ((!ragged && si == 0) ||
         (ragged && seg.off == decode_token_offset));
    // A one-token decode with every sequence live has no in-graph consumer of
    // the updated state: the next graph evaluation is the first read. Write
    // the final state directly into its persistent slab and avoid
    // materializing/copying a second S_v x S_v x H_v state. Ragged serving
    // only needs an explicit result when some rows must be excluded from
    // state write-back.
    const bool writes_every_row =
        !seg.write_slots ||
        delta_net_writes_every_row(seg.write_slots, seg.n_write_slots,
                                   seg_seqs);
    const bool inplace_state = seg_active ||
        (allow_inplace_state && can_skip_gdn_intermediate &&
         writes_every_row && n_seq_tokens == 1);
    ...
    } else if (seg.write_slots && !writes_every_row) {
        for (int wi = 0; wi < seg.n_write_slots; ++wi) {
            const int slot = seg.write_slots[wi];
            GGML_ASSERT(slot >= 0 && slot < seg_seqs);
            ggml_tensor * src = ggml_view_3d(ctx, last_conv,
                last_conv->ne[0], last_conv->ne[1], 1,
                last_conv->nb[1], last_conv->nb[2],
                (size_t)slot * last_conv->nb[2]);
            ggml_tensor * dst = ggml_view_3d(ctx, seg.conv_st,
                seg.conv_st->ne[0], seg.conv_st->ne[1], 1,
                seg.conv_st->nb[1], seg.conv_st->nb[2],
                (size_t)slot * seg.conv_st->nb[2]);
    ...
    if (!inplace_state) {
        ...
        // Persist new_state back to cache. Concurrent decode deliberately
        // skips dummy/prefilling slots so their independent recurrence is not
        // overwritten by placeholder rows.
        if (seg.write_slots && !writes_every_row) {
            for (int wi = 0; wi < seg.n_write_slots; ++wi) {
                const int slot = seg.write_slots[wi];
                GGML_ASSERT(slot >= 0 && slot < seg_seqs);
                ggml_tensor * src = ggml_view_4d(ctx, new_state,
                    new_state->ne[0], new_state->ne[1], new_state->ne[2], 1,
                    new_state->nb[1], new_state->nb[2], new_state->nb[3],
                    (size_t)slot * new_state->nb[3]);
                ggml_tensor * dst = ggml_view_4d(ctx, seg.ssm_st,
                    seg.ssm_st->ne[0], seg.ssm_st->ne[1],
                    seg.ssm_st->ne[2], 1,
                    seg.ssm_st->nb[1], seg.ssm_st->nb[2],
                    seg.ssm_st->nb[3],
                    (size_t)slot * seg.ssm_st->nb[3]);
                ggml_build_forward_expand(gf, ggml_cpy(ctx, src, dst));
            }
        } else {
    ...
    for (size_t i = 1; i < flat.size(); ++i) {
        flat_all = ggml_concat(ctx, flat_all, flat[i], 1);
    }
...
QwenGraphOutputs build_qwen35_graph(
    ...
    for (int il = 0; il < w.n_layer; il++) {
        ...
        if (is_attn) {
            ...
            cur = build_delta_net_block(ctx, gf, w, L, cur,
                                        ...
                                        in.decode_token_offset,
                                        in.state_write_slots,
                                        in.n_state_write_slots,
                                        in.active_slot_ids,
                                        in.state_slot_ids,
                                        /*allow_inplace_state=*/
                                            in.n_prefill_segments == 0);
            dn_idx++;
        }
    ...
    // 3. LM head — optionally gather selected token rows first. Concurrent
    //    steps project completed prompt tails plus decode rows, avoiding the
    //    [vocab, n_tokens] matmul for the rest of each prefill chunk.
        ...
        if (in.logits_row_indices) {
            ...
            out = ggml_view_2d(ctx, out, hidden, in.logits_tail_rows,
                               ...
                               (size_t)(n_tokens - in.logits_tail_rows) *
                                   out->nb[1]);
        } else if (in.last_token_logits_only && n_tokens > 1) {
