...
static ggml_tensor * build_full_attn_block(
    ...
    if (fused) {
        ...
        // Same 256-stride launch pad as the pure decode step (see below).
        const int launch_kv_len = paged_max_kv_len > 0 ? paged_max_kv_len : 1;
        int paged_launch_len = ((launch_kv_len + 255) / 256) * 256;
        paged_launch_len = std::min(paged_launch_len, (int)cache_k->ne[1]);
        ggml_tensor * attn_d = active_slot_ids
            ? ggml_paged_attn_active(ctx, Qfa_d, cache_k, cache_v,
                                     paged_block_table, paged_kv_seq_lens,
                                     active_slot_ids, kq_scale,
                                     PAGED_BLOCK_SIZE, paged_launch_len)
            : ggml_paged_attn(ctx, Qfa_d, cache_k, cache_v,
                              paged_block_table, paged_kv_seq_lens,
                              kq_scale, PAGED_BLOCK_SIZE,
                              paged_launch_len);
        // Paged output is [D, n_seq, Hq]; join the dense [D, Hq, tokens]
        // layout of the prefill rows on the token axis.
        attn_d = ggml_cont(ctx, ggml_permute(ctx, attn_d, 0, 2, 1, 3));
...
static ggml_tensor * build_delta_net_block(
    ...
    GGML_ASSERT(fused || (n_seqs >= 1 && n_tokens % n_seqs == 0));
    GGML_ASSERT((n_seqs == 1 && !fused) || (!cap && !parent_ids));
    ...
    if (fused) {
        ...
        segs[n_segs++] = {0, n_tokens / n_seqs, n_seqs, conv_state, ssm_state};
    }
    ...
    for (int il = 0; il < w.n_layer; il++) {
        ...
        if (is_attn) {
            ...
            if (in.n_prefill_tokens > 0) {
                ...
                // staging slabs, NOT the slot's own — fused/batched decode
                // steps between chunks dummy-write every slot slab and would
                // corrupt mid-prefill state. The final chunk (prefill_commit)
                // copies staging into the slot's slabs.
