...
struct Qwen35Config {
    ...
    // 0 = max_concurrency full contexts (no oversubscription).
...
private:
    ...
    std::vector<int64_t> paged_kv_write_rows_;
    std::vector<int32_t> paged_block_table_upload_;
    size_t paged_block_table_size_ = 0;
    ...
    int concurrent_slots() const {
        return (cfg_.paged_attention && cfg_.max_concurrency > 1)
    ...
    // needs (SeqSlotManager, slot prefill, the batched decode step).
