...
DFlashTarget * Qwen35Backend::dflash_target() {
    if (!dflash_target_) {
        dflash_target_ = std::make_unique<Qwen35DFlashTarget>(
            ...
            cfg_.device.gpu, cfg_.kq_stride_pad, cfg_.fa_window);
...
bool Qwen35Backend::do_spec_decode(int committed, int n_gen,
    ...
    // With explicit peer access, keep ordinary local chain projection entirely
    // on-device too. This makes the non-shadow control differ only by shadow
    // work, rather than by an unrelated draft-hidden host round trip.
    const bool direct_draft_projection =
        !use_remote_draft && !cfg_.ddtree_mode && split_gpus_ &&
    ...
    while (n_generated < n_gen) {
        ...
        if (use_remote_draft) {
            ...
            if (use_draft_kv) {
                ...
                draft_hidden_device = draft_kv_.hidden_states;
                if (!direct_draft_projection) {
                    local_hidden.resize((size_t)hidden * q_len);
                    ggml_backend_tensor_get(
                        draft_hidden_device, local_hidden.data(), 0,
                ...
                draft_hidden_device = draft_sg.hidden_states;
                if (!direct_draft_projection) {
                    // Legacy projection path keeps its existing host input.
                    local_hidden.resize((size_t)hidden * q_len);
                    ggml_backend_tensor_get(
                        draft_hidden_device, local_hidden.data(), 0,
                        sizeof(float) * local_hidden.size());
        ...
        if (!use_tree_verify) {
                ...
                if (!draft_hidden_device ||
                    !static_cast<Qwen35DFlashTarget *>(target)->
                        project_device_hidden_to_tokens(
                            ...
                            q_len, draft_tok)) {
                    std::fprintf(stderr,
                        "spec-decode: direct device projection failed\n");
                    step_graph_destroy(draft_sg);
                    return false;
                }
            ...
            if (async_shadow_debug) {
                std::fprintf(stderr,
                    "[async-shadow-debug] proposal step=%d committed=%d "
                    "used_shadow=%d tokens=",
                    n_draft_steps, committed, used_shadow ? 1 : 0);
                for (int tok : draft_tok) std::fprintf(stderr, "%d,", tok);
                std::fprintf(stderr, "\n");
            }
        }
        ...
        if (async_shadow_debug) {
            std::fprintf(stderr,
                "[async-shadow-debug] verify step=%d committed=%d "
                "accept_n=%d bonus=%d targets=",
                n_draft_steps, committed, accept_n, bonus_tok);
            for (int tok : target_tok) std::fprintf(stderr, "%d,", tok);
            std::fprintf(stderr, "\n");
        }
