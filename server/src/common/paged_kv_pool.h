...
// Allocation summary for callers that only need the range endpoints. Unlike
// append(), this does not materialize one PagedKvWriteSlot per token.
struct PagedKvAppendSpan {
    PagedKvStatus status = PagedKvStatus::Ok;
...
public:
    ...
    // Advance by `token_count` with the same allocation semantics as append(),
    // but return only the first and last destinations. This is the preferred
    // path for large contiguous prefills and single-token decode.
    PagedKvAppendSpan append_compact(PagedKvSequenceHandle handle,
                                     uint32_t token_count);
