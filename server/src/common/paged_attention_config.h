...
// pressure. Clamped to half the pool: the first request holds its pool
// handle for its whole lifetime, so a watermark the pool cannot spare would
// refuse every co-admission and silently serialize small explicit
// --kv-pool-tokens configurations.
inline uint32_t paged_kv_admission_watermark_blocks(
    ...
    return std::min(std::max(chunk_blocks, percent_blocks),
                    pool_blocks / 2);
}
