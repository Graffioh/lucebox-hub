// odistill_adapter.cpp — adapter file parsing, LoRA slot management, store.
...
// and drops __metadata__, which ODistill needs for the drafter-hash refusal.
...
std::vector<ODistillLoraTensorSpec> odistill_lora_expected_tensors(
    ...
    std::vector<ODistillLoraTensorSpec> v;
...
bool odistill_adapter_load(const std::string & path,
    ...
    if (meta("odistill.format") != "3") {
        error = "adapter format version mismatch (odistill.format="
              + meta("odistill.format") + ")";
        return false;
    }
    const std::string file_sha = meta("odistill.drafter_sha256");
    ...
    const std::string file_target_sha = meta("odistill.target_sha256");
    ...
    const std::string file_semantics = meta("odistill.drafter_semantics");
    ...
    if (profile.empty() || meta("odistill.profile") != profile) {
        error = "adapter profile mismatch (adapter=" +
                meta("odistill.profile") + " loaded=" + profile + ")";
        return false;
    }
    const std::string rank_s = meta("odistill.rank");
    if (rank_s != std::to_string(rank)) {
        error = "adapter rank " + rank_s + " != engine rank " +
                std::to_string(rank);
        return false;
    }
    const std::string alpha_s = meta("odistill.alpha");
    ...
    {
        const std::string g = meta("odistill.generation");
        char * end = nullptr;
        errno = 0;
        generation = std::strtoull(g.c_str(), &end, 10);
        if (g.empty() || errno != 0 || !end || *end != '\0') {
            error = "adapter has invalid odistill.generation";
            return false;
        }
    }
...
ODistillLoraWeights * odistill_lora_create(const DraftWeights & dw,
    ...
    auto * lw = new ODistillLoraWeights();
    lw->rank  = rank;
    lw->alpha = alpha;
    lw->scale = rank > 0 ? alpha / (float)rank : 0.0f;
    lw->ctx = ggml_init(ip);
    if (!lw->ctx) {
        error = "odistill lora ggml_init failed";
        delete lw;
        return nullptr;
    }
    ...
    for (int i = 0; i < dw.n_layer; i++) {
        const std::string blk = "blk." + std::to_string(i);
        ODistillLoraLayer & L = lw->layers[(size_t)i];
    ...
    if (!lw->buf) {
        error = "odistill lora buffer alloc failed";
        odistill_lora_free(lw);
        return nullptr;
    }
    // Zero-fill so serving before the first adapter equals the base drafter.
    ODistillAdapterHost zeros;  // empty tensors map → zero upload
    if (!odistill_lora_upload(*lw, zeros, error)) {
        odistill_lora_free(lw);
        return nullptr;
    }
    return lw;
