// odistill_supervisor.cpp — trainer sidecar spawn/supervise/relay.

#include "odistill_supervisor.h"
...
bool ODistillSupervisor::start(const ODistillSupervisorConfig &) { return false; }
void ODistillSupervisor::stop() {}
bool ODistillSupervisor::trainer_alive() const { return false; }
bool ODistillSupervisor::trainer_disabled() const { return false; }
uint64_t ODistillSupervisor::respawns() const { return 0; }
bool ODistillSupervisor::take_pending_swap(ODistillPendingSwap &) { return false; }
bool ODistillSupervisor::clear_pending_swaps() { return false; }
void ODistillSupervisor::begin_rollback(uint64_t) {}
bool ODistillSupervisor::rollback_pending() const { return false; }
void ODistillSupervisor::send_line(const std::string &) {}
void ODistillSupervisor::run() {}
bool ODistillSupervisor::spawn_once(std::string &) { return false; }
void ODistillSupervisor::reap_child(bool) {}
void ODistillSupervisor::drain_outbox_locked() {}
...
bool ODistillSupervisor::start(const ODistillSupervisorConfig & cfg) {
    ...
    try {
        ...
        std::fprintf(stderr, "[odistill] supervisor thread spawn failed\n");
        return false;
    }
    return true;
}
...
bool ODistillSupervisor::trainer_alive() const {
    std::lock_guard<std::mutex> lock(mu_);
    return alive_;
}

bool ODistillSupervisor::trainer_disabled() const {
    std::lock_guard<std::mutex> lock(mu_);
    return trainer_disabled_;
}

uint64_t ODistillSupervisor::respawns() const {
    std::lock_guard<std::mutex> lock(mu_);
    return respawns_;
}

bool ODistillSupervisor::take_pending_swap(ODistillPendingSwap & out) {
    std::lock_guard<std::mutex> lock(mu_);
...
bool ODistillSupervisor::clear_pending_swaps() {
    std::lock_guard<std::mutex> lock(mu_);
...
void ODistillSupervisor::begin_rollback(uint64_t generation) {
    std::lock_guard<std::mutex> lock(mu_);
    pending_ = {};
    has_pending_ = false;
...
bool ODistillSupervisor::rollback_pending() const {
    std::lock_guard<std::mutex> lock(mu_);
    return rollback_pending_;
}

void ODistillSupervisor::send_line(const std::string & line) {
    std::lock_guard<std::mutex> lock(mu_);
    outbox_.push_back(line);
}

bool ODistillSupervisor::spawn_once(std::string & error) {
    ...
    if (pid == 0) {
        ...
        std::fprintf(stderr, "[odistill-trainer] execv %s failed: %s\n",
...
void ODistillSupervisor::reap_child(bool graceful) {
...
void ODistillSupervisor::run() {
    ...
    while (true) {
        ...
        std::string error;
        if (!spawn_once(error)) {
            std::fprintf(stderr, "[odistill] trainer spawn failed: %s\n",
                         error.c_str());
            attempts++;
            if (attempts > (uint64_t)cfg_.max_respawns) {
                std::fprintf(stderr,
                    "[odistill] trainer respawn limit reached; capture-only\n");
                break;
            }
        ...
        std::fprintf(stderr, "[odistill] trainer ready (pid=%ld)\n", child_pid_);
        ...
        while (!child_eof) {
            ...
            while ((nl = line_buf.find('\n')) != std::string::npos) {
                ...
                if (ev == "swap_ready") {
                    ...
                    if (rollback_pending_) {
                        std::fprintf(stderr,
                            "[odistill] discarding adapter announcement during "
                            "rollback barrier\n");
                    } else {
                    ...
                    std::fprintf(stderr,
                        "[odistill-trainer] training disabled: %s\n",
                        j.value("reason", "unspecified").c_str());
                } else if (ev == "log") {
                    std::fprintf(stderr, "[odistill-trainer] %s\n",
        ...
        std::fprintf(stderr, "[odistill] trainer exited; will respawn\n");
        reap_child(false);
        attempts++;
        if (attempts > (uint64_t)cfg_.max_respawns) {
            std::fprintf(stderr,
