// odistill_runtime.h — the engine-side ODistill orchestrator.
//
// Owned by the (qwen35) backend when --odistill is on. Composes:
//   ODistillRing        capture stream to the trainer (shm, drop-on-full)
//   ODistillLoraWeights pointer-stable adapter tensors wired into DraftWeights
//   ODistillGuard       acceptance guard state machine
//   ODistillSupervisor  trainer sidecar lifecycle + control lines
...
#include "odistill_config.h"
#include "odistill_guard.h"
#include "odistill_lora.h"
#include "odistill_props.h"
...
inline constexpr bool odistill_generation_is_newer(uint64_t candidate,
                                                 uint64_t resident,
                                                 uint64_t pending = 0,
...
// pimpl holder so this header need not include odistill_adapter.h.
...
struct ODistillStepCapture {
    ...
    const uint8_t * accept_flags = nullptr; // [block] 1/0/2 per odistill_format.h
...
public:
    ...
    ODistillRuntime() = default;
    ~ODistillRuntime() { shutdown(); }
    ODistillRuntime(const ODistillRuntime &) = delete;
    ODistillRuntime & operator=(const ODistillRuntime &) = delete;

    // Hashes the drafter, creates ring + LoRA slots (wired into dw.odistill),
    ...
    // then disables ODistill while leaving inference available.
    bool init(const ODistillConfig & cfg,
    ...
    void shutdown();  // detach dw.odistill BEFORE freeing the draft backend
    ...
    void emit_step(const ODistillStepCapture & s, const FeatReader & read);
    ...
    // ── Stats (any thread) ──────────────────────────────────────────
    ODistillPropsSnapshot props() const;

private:
    void apply_guard_action(ODistillGuardAction action);
    ...
    ODistillRing ring_;
    ODistillGuard guard_{};
    ODistillSupervisor supervisor_;
    ODistillLoraWeights * lora_ = nullptr;

    // Host staging: current + previous generation for guard rollback.
    ODistillAdapterHostHolder * staging_ = nullptr;  // pimpl (adapter header)
    ...
    ODistillPendingSwap pending_local_;
    ...
    ODistillPropsSnapshot props_cache_;
    uint64_t steps_ = 0;
};
