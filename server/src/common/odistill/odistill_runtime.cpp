// odistill_runtime.cpp — ODistill orchestrator implementation.

#include "odistill_runtime.h"
#include "odistill_adapter.h"
#include "odistill_format.h"

#include "internal.h"               // DraftWeights
#include "common/gguf_inspect.h"    // read_gguf_metadata (drafter sha256)

#include <algorithm>
#include <cerrno>
#include <cmath>
#include <cstdio>
#include <cstdlib>
#include <cstring>
#include <ctime>
#include <iomanip>
#include <limits>
#include <new>
#include <sstream>

#if !defined(_WIN32)
#include <sys/stat.h>
#include <unistd.h>
#endif

namespace dflash::common::odistill {

struct ODistillAdapterHostHolder {
    ODistillAdapterHost current;
    ODistillAdapterHost previous;
    std::string current_path;
};

namespace {

uint64_t mono_ns() {
#if defined(_WIN32)
    return 0;
#else
    struct timespec ts{};
    clock_gettime(CLOCK_MONOTONIC, &ts);
    return (uint64_t)ts.tv_sec * 1000000000ull + (uint64_t)ts.tv_nsec;
#endif
}

bool make_dirs(const std::string & path) {
#if defined(_WIN32)
    (void)path;
    return false;
#else
    std::string cur;
    for (size_t i = 0; i < path.size(); i++) {
        cur += path[i];
        if (path[i] == '/' || i + 1 == path.size()) {
            if (cur == "/" || cur.empty()) continue;
            if (::mkdir(cur.c_str(), 0700) != 0 && errno != EEXIST) {
                return false;
            }
        }
    }
    return true;
#endif
}

std::string resolved_swa_pattern(const DraftWeights & dw,
                                 char separator = ',') {
    std::string pattern;
    pattern.reserve((size_t)std::max(0, dw.n_layer) * 2);
    for (int il = 0; il < dw.n_layer; ++il) {
        if (il > 0 && separator != '\0') pattern.push_back(separator);
        pattern.push_back(dw.layers[(size_t)il].is_swa ? '1' : '0');
    }
    return pattern;
}

std::string resolved_drafter_semantics_impl(const DraftWeights & dw) {
    uint32_t rope_bits = 0;
    static_assert(sizeof(rope_bits) == sizeof(dw.rope_theta));
    std::memcpy(&rope_bits, &dw.rope_theta, sizeof(rope_bits));
    std::ostringstream out;
    out << "v1;rope=" << std::hex << std::setfill('0') << std::setw(8)
        << rope_bits << std::dec
        << ";swa=" << dw.swa_window
        << ";pattern=" << resolved_swa_pattern(dw, '\0')
        << ";mask=" << dw.mask_token_id;
    return out.str();
}

std::string resolved_float_arg(float value) {
    std::ostringstream out;
    out << std::setprecision(std::numeric_limits<float>::max_digits10)
        << value;
    return out.str();
}

std::string semantics_tag_impl(const std::string & value) {
    // Stable FNV-1a (std::hash is not a persistence contract). The full
    // semantics string remains in adapter metadata and is checked on load;
    // this short tag is only a collision-resistant directory namespace.
    uint64_t hash = UINT64_C(14695981039346656037);
    for (unsigned char c : value) {
        hash ^= c;
        hash *= UINT64_C(1099511628211);
    }
    std::ostringstream out;
    out << std::hex << std::setfill('0') << std::setw(16) << hash;
    return out.str();
}

const char * incompatible_debug_override() {
    static constexpr const char * names[] = {
        "DFLASH_DISABLE_DRAFT_AUX_NORMS",
        "DFLASH_DISABLE_DRAFT_ATTN_GATE",
        "DFLASH_DISABLE_DRAFT_SWA",
        "DFLASH_DISABLE_DRAFT_ATTN",
        "DFLASH_DISABLE_DRAFT_FFN",
    };
    for (const char * name : names) {
        if (std::getenv(name) != nullptr) return name;
    }
    return nullptr;
}

}  // namespace

std::string odistill_resolved_drafter_semantics(const DraftWeights & dw) {
    return resolved_drafter_semantics_impl(dw);
}

std::string odistill_semantics_tag(const std::string & value) {
    return semantics_tag_impl(value);
}

std::string odistill_adapter_contract(const std::string & drafter_sha256,
                                    const std::string & target_sha256,
                                    const std::string & drafter_semantics,
                                    int rank,
                                    float alpha) {
    uint32_t alpha_bits = 0;
    static_assert(sizeof(alpha_bits) == sizeof(alpha));
    std::memcpy(&alpha_bits, &alpha, sizeof(alpha_bits));
    std::ostringstream out;
    out << "drafter=" << drafter_sha256
        << ";target=" << target_sha256
        << ";draft=" << drafter_semantics
        << ";rank=" << rank
        << ";alpha=" << std::hex << std::setfill('0') << std::setw(8)
        << alpha_bits;
    return out.str();
}

bool ODistillRuntime::init(const ODistillConfig & cfg,
                         const std::string & target_path,
                         const std::string & drafter_path,
                         DraftWeights & dw,
                         ggml_backend_t draft_backend,
                         int target_capture_layers,
                         int target_hidden,
                         int vocab) {
    cfg_ = cfg;

    if (cfg.lora_rank <= 0 || cfg.lora_rank > 256 ||
        !std::isfinite(cfg.lora_alpha) || cfg.lora_alpha <= 0.0f) {
        std::fprintf(stderr, "[odistill] invalid LoRA rank/alpha; disabled\n");
        return false;
    }
    if (dw.n_layer <= 0 || dw.layers.size() != (size_t)dw.n_layer ||
        !std::isfinite(dw.rope_theta) || dw.rope_theta <= 0.0f ||
        dw.swa_window < 0) {
        std::fprintf(stderr,
            "[odistill] invalid resolved drafter metadata; disabled\n");
        return false;
    }
    if (const char * override_name = incompatible_debug_override()) {
        std::fprintf(stderr,
            "[odistill] %s changes the served draft graph but is not mirrored "
            "by the trainer; disabled\n", override_name);
        return false;
    }
    drafter_semantics_ = odistill_resolved_drafter_semantics(dw);

    // The capture callback copies target-owned feature rows while record sizing
    // is derived from the drafter's fc input. A mismatch would make that copy
    // overrun its staging vector, so refuse ODistill before allocating any slots.
    if (target_capture_layers <= 0 || target_hidden <= 0 ||
        dw.n_target_layers != target_capture_layers ||
        dw.n_embd != target_hidden) {
        std::fprintf(stderr,
            "[odistill] incompatible target/draft features: target=%dx%d "
            "draft=%dx%d; disabled\n",
            target_capture_layers, target_hidden,
            dw.n_target_layers, dw.n_embd);
        return false;
    }

    staging_ = new (std::nothrow) ODistillAdapterHostHolder();
    if (!staging_) {
        std::fprintf(stderr,
            "[odistill] host staging allocation failed; disabled\n");
        return false;
    }
    dw_ = &dw;

    // 1. LoRA slots — the only hard requirement. Without them there is no
    //    adaptation, so a failure here disables ODistill entirely.
    std::string error;
    lora_ = odistill_lora_create(dw, draft_backend, cfg.lora_rank,
                               cfg.lora_alpha, error);
    if (!lora_) {
        std::fprintf(stderr, "[odistill] init failed: %s\n", error.c_str());
        return false;
    }
    dw.odistill = lora_;

    // 2. Model identity (sidecar-cached SHA-256 of each GGUF).
    const GgufMetadata drafter_meta = read_gguf_metadata(drafter_path, true);
    if (!drafter_meta.ok || drafter_meta.sha256.empty()) {
        std::fprintf(stderr,
            "[odistill] drafter hash failed for %s; adapters disabled, "
            "capture continues unlabeled\n", drafter_path.c_str());
    } else {
        drafter_sha256_ = drafter_meta.sha256;
    }
    const GgufMetadata target_meta = read_gguf_metadata(target_path, true);
    if (!target_meta.ok || target_meta.sha256.empty()) {
        std::fprintf(stderr,
            "[odistill] target hash failed for %s; adapters disabled, "
            "capture continues unlabeled\n", target_path.c_str());
    } else {
        target_sha256_ = target_meta.sha256;
    }

    // 3. Profile store + warm start (ODISTILL.md §8).
    if (!drafter_sha256_.empty() && !target_sha256_.empty()) {
        const std::string semantic_profile =
            cfg.profile + "-sem-" + odistill_semantics_tag(
                odistill_adapter_contract(drafter_sha256_, target_sha256_,
                                         drafter_semantics_,
                                         cfg.lora_rank, cfg.lora_alpha));
        profile_dir_ = odistill_profile_dir(cfg.dir, drafter_sha256_,
                                          semantic_profile);
        if (!make_dirs(profile_dir_)) {
            std::fprintf(stderr, "[odistill] cannot create %s; persistence off\n",
                         profile_dir_.c_str());
            profile_dir_.clear();
        } else {
            std::fprintf(stderr, "[odistill] profile dir %s\n",
                         profile_dir_.c_str());
        }
        std::string warm_path;
        uint64_t warm_gen = 0;
        if (!profile_dir_.empty() &&
            odistill_store_read_promoted(profile_dir_, warm_path, warm_gen)) {
            if (load_and_upload(warm_path, warm_gen, /*warm_start=*/true)) {
                std::fprintf(stderr,
                    "[odistill] warm start: adapter gen %llu (%s)\n",
                    (unsigned long long)warm_gen, warm_path.c_str());
            }
        }
    }
    generation_high_water_ = lora_->generation;

    // 4. Capture ring.
#if !defined(_WIN32)
    ring_name_ = "/lucebox-odistill-" + std::to_string((long)::getpid());
#endif
    const uint64_t ring_bytes = (uint64_t)cfg.ring_mb << 20;
    const std::string drafter_semantics_tag =
        odistill_semantics_tag(drafter_semantics_);
    if (!ring_.create(ring_name_, ring_bytes,
                      odistill_hash_from_hex(drafter_sha256_.c_str()),
                      odistill_hash_from_hex(target_sha256_.c_str()),
                      odistill_hash_from_hex(drafter_semantics_tag.c_str()),
                      (uint32_t)dw.n_target_layers,
                      (uint32_t)dw.n_embd,
                      (uint32_t)dw.block_size,
                      (uint32_t)cfg.topk,
                      (uint32_t)vocab)) {
        std::fprintf(stderr, "[odistill] ring create failed; capture off\n");
    } else {
        std::fprintf(stderr, "[odistill] capture ring %s (%d MiB)\n",
                     ring_name_.c_str(), cfg.ring_mb);
    }

    // 5. Trainer sidecar (optional — empty bin = M0 capture-only).
    if (!cfg.trainer_bin.empty() && ring_.active() &&
        !drafter_sha256_.empty() && !target_sha256_.empty() &&
        !profile_dir_.empty()) {
        ODistillSupervisorConfig scfg;
        scfg.trainer_bin  = cfg.trainer_bin;
        scfg.drafter_path = drafter_path;
        scfg.args = {
            "--ring-name=" + ring_name_,
            "--target=" + target_path,
            "--out-dir=" + profile_dir_,
            "--profile=" + cfg.profile,
            "--rank=" + std::to_string(cfg.lora_rank),
            "--alpha=" + resolved_float_arg(cfg.lora_alpha),
            "--device=" + cfg.device,
            "--dtype=" + cfg.dtype,
            "--resolved-rope-theta=" + resolved_float_arg(dw.rope_theta),
            "--resolved-swa-window=" + std::to_string(dw.swa_window),
            "--resolved-swa-pattern=" + resolved_swa_pattern(dw),
            "--resolved-mask-token-id=" + std::to_string(dw.mask_token_id),
            "--drafter-semantics=" + drafter_semantics_,
            "--drafter-sha256=" + drafter_sha256_,
            "--target-sha256=" + target_sha256_,
            "--start-generation=" + std::to_string(lora_->generation),
        };
        if (!supervisor_.start(scfg)) {
            std::fprintf(stderr,
                "[odistill] trainer not started; capture-only mode\n");
        }
    } else if (cfg.trainer_bin.empty()) {
        std::fprintf(stderr, "[odistill] no --odistill-trainer-bin; "
                             "capture-only (M0) mode\n");
    }
    refresh_props_cache();  // warm-start generation visible before step 1
    return true;
}

void ODistillRuntime::shutdown() {
    supervisor_.stop();
    ring_.close();
    if (dw_) dw_->odistill = nullptr;
    if (lora_) { odistill_lora_free(lora_); lora_ = nullptr; }
    delete staging_;
    staging_ = nullptr;
    dw_ = nullptr;
}

void ODistillRuntime::on_request_begin() {
    seq_id_++;
    want_context_ = ring_.active();
}

void ODistillRuntime::on_request_end(int committed) {
    if (!ring_.active()) return;
    ODistillRecordHeader h{};
    h.type = ODISTILL_REC_SEQ_END;
    h.seq_id = seq_id_;
    h.pos = committed;
    h.n_rows = 0;
    h.t_mono_ns = mono_ns();
    ring_.push(h, nullptr, 0);
}

void ODistillRuntime::emit_context(int pos0, int n_rows,
                                 const FeatReader & read) {
    want_context_ = false;
    if (!ring_.active() || n_rows <= 0 || !dw_) return;
    try {
        const size_t row_elems = (size_t)dw_->n_target_layers * dw_->n_embd;
        std::vector<uint16_t> feat((size_t)n_rows * row_elems);
        for (int i = 0; i < n_rows; i++) {
            if (!read(pos0 + i, feat.data() + (size_t)i * row_elems)) return;
        }
        ODistillRecordHeader h{};
        h.type = ODISTILL_REC_CONTEXT;
        h.seq_id = seq_id_;
        h.pos = pos0;
        h.n_rows = n_rows;
        h.t_mono_ns = mono_ns();
        ring_.push(h, feat.data(), feat.size() * sizeof(uint16_t));
    } catch (const std::bad_alloc &) {
        // Capture is optional. In particular, never let UMA/host pressure turn
        // a backfill allocation failure into a failed inference request.
        std::fprintf(stderr,
            "[odistill] context capture allocation failed; record dropped\n");
    }
}

void ODistillRuntime::emit_step(const ODistillStepCapture & s,
                              const FeatReader & read) {
    steps_++;
    // Guard first — AL bookkeeping must run even when the ring is off.
    apply_guard_action(guard_.record_step((float)s.accept_len));
    refresh_props_cache();

    if (!ring_.active() || !dw_) return;

    try {
        const size_t row_elems = (size_t)dw_->n_target_layers * dw_->n_embd;
        std::vector<uint16_t> feat((size_t)s.n_rows * row_elems);
        for (int i = 0; i < s.n_rows; i++) {
            if (!read(s.pos + i, feat.data() + (size_t)i * row_elems)) return;
        }

        // Label section, mirroring the STEP payload layout in odistill_format.h.
        const int32_t B = s.block;
        const int32_t K = (s.topk_ids && s.topk_lp) ? cfg_.topk : 0;
        const size_t flags_padded = ((size_t)B + 7) & ~(size_t)7;
        const size_t label_bytes = 8                 // n_labels + topk_k
                           + (size_t)B * 4 * 2       // draft_tok + target_tok
                           + flags_padded            // accept_flags (+pad)
                           + 8                       // accept_len + bonus_tok
                           + (size_t)B * (size_t)K * 8;
        std::vector<uint8_t> labels(label_bytes, 0);
        uint8_t * p = labels.data();
        std::memcpy(p, &B, 4);                       p += 4;
        std::memcpy(p, &K, 4);                       p += 4;
        if (B > 0) {
            std::memcpy(p, s.draft_tok, (size_t)B * 4);  p += (size_t)B * 4;
            std::memcpy(p, s.target_tok, (size_t)B * 4); p += (size_t)B * 4;
            std::memcpy(p, s.accept_flags, (size_t)B);   p += flags_padded;
        }
        std::memcpy(p, &s.accept_len, 4);            p += 4;
        std::memcpy(p, &s.bonus_tok, 4);             p += 4;
        if (K > 0) {
            std::memcpy(p, s.topk_ids, (size_t)B * K * 4); p += (size_t)B * K * 4;
            std::memcpy(p, s.topk_lp,  (size_t)B * K * 4); p += (size_t)B * K * 4;
        }

        ODistillRecordHeader h{};
        h.type = ODISTILL_REC_STEP;
        h.seq_id = seq_id_;
        h.pos = s.pos;
        h.n_rows = s.n_rows;
        h.t_mono_ns = mono_ns();
        ring_.push2(h, feat.data(), feat.size() * sizeof(uint16_t),
                    labels.data(), labels.size());
    } catch (const std::bad_alloc &) {
        std::fprintf(stderr,
            "[odistill] step capture allocation failed; record dropped\n");
    }
}

void ODistillRuntime::maybe_apply_swap() {
    if (training_disabled_ || !lora_) return;
    ODistillPendingSwap swap;
    if (supervisor_.take_pending_swap(swap)) {
        const uint64_t pending_generation =
            has_pending_local_ ? pending_local_.generation : 0;
        if (odistill_generation_is_newer(swap.generation, lora_->generation,
                                       pending_generation,
                                       generation_high_water_)) {
            // A newer trainer export supersedes any deferred one.
            pending_local_ = swap;
            has_pending_local_ = true;
            generation_high_water_ = swap.generation;
        } else {
            std::fprintf(stderr,
                "[odistill] ignoring nonmonotonic adapter gen %llu "
                "(resident=%llu pending=%llu high-water=%llu)\n",
                (unsigned long long)swap.generation,
                (unsigned long long)lora_->generation,
                (unsigned long long)pending_generation,
                (unsigned long long)generation_high_water_);
        }
    }
    if (!has_pending_local_ || !guard_.can_swap()) return;
    if (!odistill_generation_is_newer(pending_local_.generation,
                                    lora_->generation)) {
        has_pending_local_ = false;
        return;
    }
    has_pending_local_ = false;
    if (load_and_upload(pending_local_.path, pending_local_.generation,
                        /*warm_start=*/false)) {
        guard_.on_swap(pending_local_.generation);
    }
    refresh_props_cache();
}

void ODistillRuntime::apply_guard_action(ODistillGuardAction action) {
    switch (action) {
        case ODistillGuardAction::None:
            break;
        case ODistillGuardAction::Promote: {
            std::fprintf(stderr,
                "[odistill] promote gen %llu (AL %.2f -> %.2f)\n",
                (unsigned long long)lora_->generation,
                guard_.decision_baseline_al(),
                guard_.decision_probation_al());
            if (!profile_dir_.empty() && staging_ &&
                !staging_->current.path.empty()) {
                odistill_store_write_promoted(profile_dir_,
                                            staging_->current.path,
                                            lora_->generation);
            }
            supervisor_.send_line("promote " +
                                  std::to_string(lora_->generation));
            break;
        }
        case ODistillGuardAction::Rollback:
        case ODistillGuardAction::Disable: {
            const uint64_t bad_gen = lora_ ? lora_->generation : 0;
            // Any queued adapter was trained from (or announced while serving)
            // the rejected generation. It must not survive the rollback.
            pending_local_ = {};
            has_pending_local_ = false;
            if (action == ODistillGuardAction::Rollback) {
                supervisor_.begin_rollback(bad_gen);
            } else {
                supervisor_.clear_pending_swaps();
            }
            std::fprintf(stderr,
                "[odistill] %s gen %llu (probation AL %.2f < baseline %.2f)\n",
                action == ODistillGuardAction::Disable ? "DISABLE after rollback of"
                                                     : "rollback",
                (unsigned long long)bad_gen,
                guard_.decision_probation_al(),
                guard_.decision_baseline_al());
            // Restore the previous generation's tensors from host staging.
            if (lora_ && staging_) {
                std::string err;
                if (!odistill_lora_upload(*lora_, staging_->previous, err)) {
                    // Previous was the zero adapter (empty tensors) or a
                    // partial staging failure — fall back to zeros.
                    ODistillAdapterHost zeros;
                    if (!odistill_lora_upload(*lora_, zeros, err)) {
                        std::fprintf(stderr,
                            "[odistill] rollback upload failed: %s\n",
                            err.c_str());
                    }
                }
                staging_->current = staging_->previous;
                adapter_dirty_ = true;
            }
            if (action == ODistillGuardAction::Disable) {
                supervisor_.send_line("disable");
                training_disabled_ = true;
            }
            break;
        }
    }
}

bool ODistillRuntime::load_and_upload(const std::string & path,
                                    uint64_t generation,
                                    bool warm_start) {
    if (!lora_ || !dw_ || !staging_) return false;
    ODistillAdapterHost host;
    std::string error;
    if (!odistill_adapter_load(path, *dw_, cfg_.lora_rank, cfg_.lora_alpha,
                             drafter_sha256_, target_sha256_,
                             drafter_semantics_, cfg_.profile,
                             host, error)) {
        std::fprintf(stderr, "[odistill] adapter refused (%s): %s\n",
                     path.c_str(), error.c_str());
        return false;
    }
    if (generation != 0 && host.generation != generation) {
        std::fprintf(stderr,
            "[odistill] adapter %s generation %llu != announced %llu\n",
            path.c_str(), (unsigned long long)host.generation,
            (unsigned long long)generation);
        return false;
    }
    if (!warm_start &&
        !odistill_generation_is_newer(host.generation, lora_->generation)) {
        std::fprintf(stderr,
            "[odistill] adapter %s generation %llu does not advance resident "
            "generation %llu\n",
            path.c_str(), (unsigned long long)host.generation,
            (unsigned long long)lora_->generation);
        return false;
    }
    if (!odistill_lora_upload(*lora_, host, error)) {
        std::fprintf(stderr, "[odistill] adapter upload failed: %s\n",
                     error.c_str());
        return false;
    }
    if (!warm_start) staging_->previous = staging_->current;
    staging_->current = host;
    adapter_dirty_ = true;
    return true;
}

// Decode thread only; rebuilds the HTTP-visible snapshot after any guard or
// adapter mutation.
void ODistillRuntime::refresh_props_cache() {
    ODistillPropsSnapshot s;
    s.enabled = true;
    s.profile = cfg_.profile;
    s.adapter_generation = lora_ ? lora_->generation : 0;
    s.swaps = guard_.swaps();
    s.promotes = guard_.promotes();
    s.rollbacks = guard_.rollbacks();
    s.rolling_al = guard_.baseline_al();
    s.in_probation = guard_.state() == ODistillGuard::State::Probation;
    s.probation_al = s.in_probation ? guard_.probation_al() : 0.0;
    s.training_disabled =
        training_disabled_ || guard_.state() == ODistillGuard::State::Disabled;
    s.records_written = ring_.written();
    s.steps = steps_;
    std::lock_guard<std::mutex> lock(stats_mu_);
    props_cache_ = s;
}

ODistillPropsSnapshot ODistillRuntime::props() const {
    ODistillPropsSnapshot s;
    {
        std::lock_guard<std::mutex> lock(stats_mu_);
        s = props_cache_;
    }
    s.enabled = true;
    s.profile = cfg_.profile;
    // Cross-thread-safe live values (shm atomics / supervisor mutex).
    s.trainer_alive = supervisor_.trainer_alive();
    s.training_disabled =
        s.training_disabled || supervisor_.trainer_disabled();
    s.records_dropped = ring_.dropped();
    s.ring_backlog_bytes = ring_.backlog();
    return s;
}

}  // namespace dflash::common::odistill
