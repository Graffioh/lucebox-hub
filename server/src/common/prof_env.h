...
//
// LUCE_PROF is a comma-separated list of profiler names, replacing the
// per-profiler env-var sprawl. Example: LUCE_PROF=step,verify,prefill
...
inline bool dflash_prof_enabled(const char * name) {
    const char * e = std::getenv("LUCE_PROF");
    if (!e) return false;
