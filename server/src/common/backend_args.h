...
struct BackendFeatureConfig {
    ...
    // KVFlash bounded KV residency, enabled through DFLASH_KVFLASH rather than
    // a flag. Recorded here because the gate cannot read the environment: it
    // is linked without the pool sizing code so it stays a pure function.
