...
DFlash2DepthSignal summarize_depth(
    ...
    for (int k = 0; k < K; ++k) {
        if (k != selected) {
            runner_up = std::max(runner_up, scores[(size_t) k]);
        }
    }
...
bool dflash2_select_chains_batched(
    ...
    static const bool gpu_top_k = []() {
        const char * value = std::getenv("DFLASH_GPU_DRAFT_TOPK");
        return value == nullptr || value[0] != '0';
    }();
    if (gpu_top_k && projection.logits && projection.logits->data) {
    ...
    if (traces) traces->resize((size_t) n_lanes);
    for (int lane = 0; lane < n_lanes; ++lane) {
        draft_tokens[(size_t) lane][0] = last_tokens[(size_t) lane];
        if (traces) {
            (*traces)[(size_t) lane].depths.reserve((size_t) n_cand);
        }
        ...
        for (int depth = 0; depth < n_cand; ++depth) {
            ...
            if (traces) {
                (*traces)[(size_t) lane].depths.push_back(
                    summarize_depth(
                        candidate_log_probs.data() +
                            (size_t) position * (size_t) K,
                        scores, K, best_candidate));
            }
