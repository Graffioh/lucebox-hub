// Uncertainty-gated packed branching for exact DDTree verification.
//
...
constexpr int DDTREE_ADAPTIVE_BASELINE_NODES = 15;
constexpr int DDTREE_ADAPTIVE_BRANCHES = 3;
constexpr int DDTREE_ADAPTIVE_BRANCH_NODES = 5;
constexpr int DDTREE_ADAPTIVE_ALLOC_ROWS = 32;
...
// continuation. A round branches only when margin < threshold; equality stays
// on the existing chain fast path.
...
//   root + complete 15-node baseline + 3 independent 5-node alternatives.
// Branch paths are branch-major. Returns a root-only tree on invalid input.
...
int ddtree_adaptive_required_rows(
    int baseline_nodes = DDTREE_ADAPTIVE_BASELINE_NODES,
    int branch_count = DDTREE_ADAPTIVE_BRANCHES,
    int branch_nodes = DDTREE_ADAPTIVE_BRANCH_NODES,
    int tile = DDTREE_ADAPTIVE_ALLOC_ROWS);
