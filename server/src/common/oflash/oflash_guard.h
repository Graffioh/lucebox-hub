// oflash_guard.h — acceptance guard state machine (OFLASH.md §7).
//
...
public:
    ...
    explicit OFlashGuard(const OFlashGuardConfig & cfg = {})
    ...
    OFlashGuardAction record_step(float accept_len) {
        steps_total_++;
        if (state_ == State::Disabled) return OFlashGuardAction::None;

        if (state_ == State::Serving) {
            push_window(baseline_, accept_len, cfg_.baseline_window);
            return OFlashGuardAction::None;
        }
        ...
        if (pass) {
            ...
            promotes_++;
            return OFlashGuardAction::Promote;
        }
        probation_.clear();
        ...
        if (consecutive_rollbacks_ >= cfg_.max_consecutive_rollbacks) {
            ...
            return OFlashGuardAction::Disable;
        }
        return OFlashGuardAction::Rollback;
    }
