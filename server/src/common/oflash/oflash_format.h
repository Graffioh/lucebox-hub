// oflash_format.h — versioned wire/file formats shared between the engine
...
//      here; parsing lives in oflash_adapter.cpp / the Python exporter.
//
// The Python mirror of these structs is optimizations/oflash/src/oflash/
// ring_format.py. Any change here must bump OFLASH_RING_VERSION and be
// mirrored there; test/test_oflash_unit.cpp writes a fixture ring that the
...
struct OFlashRingHeader {
    ...
    uint32_t version;          // OFLASH_RING_VERSION
...
// ── Record framing ──────────────────────────────────────────────────
//
// Every record starts with OFlashRecordHeader. `size_bytes` counts the
...
enum OFlashRecordType : uint32_t {
    OFLASH_REC_PAD     = 0,  // filler to the end of the buffer; skip
    OFLASH_REC_CONTEXT = 1,  // feature backfill rows, no labels
...
static_assert(sizeof(OFlashRecordHeader) == 32, "keep layout stable");
