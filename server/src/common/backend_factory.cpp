...
// So the qwen35moe row gets pinned instead of derived. Qwen35MoeBackend
// inherits the init() that builds the paged pool but overrides
// run_ar_decode_path() with a pipelined path that never reads a block table,
// so the flag would allocate a pool and decode dense. Flipping this row needs
// that decode path to learn paging first.
static_assert(arch_capabilities("qwen35moe").paged_attn == FeatureSupport::Never,
              "qwen35moe shares Qwen35Config but its decode path ignores "
              "paged_attention; the capability row must stay Never");
