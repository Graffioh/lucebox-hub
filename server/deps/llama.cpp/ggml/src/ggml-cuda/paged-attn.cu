...
static __global__ void paged_attn_decode(
    ...
    const int32_t tree_seq = tree_mode ? seq / tree_width : 0;
    const int32_t query_node =
        tree_mode ? seq - tree_seq * tree_width : -1;
    const int32_t tree_size = tree_mode
        ? *(const int32_t *) (
    ...
    const int32_t virtual_tokens = valid_query
        ? kv_seq_len + (tree_mode ? tree_width : 0)
        : 0;
...
#pragma unroll
    ...
    for (int32_t tile_begin = token_begin;
        ...
        } else if (tree_mode && my_token < token_end) {
...
bool ggml_cuda_paged_attn_supported(const ggml_tensor * dst) {
    ...
    if ((parent_ids == nullptr) != (tree_sizes == nullptr) ||
        (tree_mode && (!active_slot_ids || query_positions))) {
        return false;
    }
