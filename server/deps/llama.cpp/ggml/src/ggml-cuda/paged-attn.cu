...
#pragma unroll
                for (int i = 0; i < values_per_load; ++i) {
                    partial_acc[partial_row * D + value0 + i] =
                        __float2half(acc[h][segment * values_per_load + i] * inv_sum);
                }
            }
            if (lane == 0) {
...
static bool try_launch_paged_attn(
    ...
    while (true) {
        ...
        std::atomic<int> & cached =
            occupancy[ctx.device][warps_per_block];
        ...
        if (probe == 0) {
            ...
            // the status has to drive the fallback below instead of aborting.
        ...
        max_blocks_per_sm = probe > 0 ? probe : 0;
    ...
    if (n_partitions > live_blocks) {
        n_partitions = live_blocks;
    }
    ...
    ggml_cuda_pool_alloc<half> acc_scratch(
        ctx.pool(), partial_values);
    ggml_cuda_pool_alloc<float2> meta_scratch(
        ctx.pool(), partial_rows);
    half   * partial_acc  = acc_scratch.ptr;
    float2 * partial_meta = meta_scratch.ptr;
...
static void launch_paged_attn(
    ...
    // Debug/tuning override to pin the per-warp head batch (1, 3, or 6) so
    // bench_paged_attention can A/B the widths directly. Read once — this
    // sits in the per-layer decode hot path.
    static const int forced_batch_heads = []() {
        const char * env =
            std::getenv("GGML_CUDA_PAGED_ATTN_BATCH_HEADS");
        return env ? std::atoi(env) : 0;
    }();
    const int batch_heads_limit =
        forced_batch_heads >= 1 ? forced_batch_heads : 6;
    ...
    if (gqa_ratio % 6 == 0 && batch_heads_limit >= 6 &&
        try_launch_paged_attn<type_K, type_V, 6>(
            ctx, dst, scale, block_size, max_kv_seq_len)) {
        return;
    }
    if (gqa_ratio % 3 == 0 && batch_heads_limit >= 3 &&
        try_launch_paged_attn<type_K, type_V, 3>(
            ctx, dst, scale, block_size, max_kv_seq_len)) {
        return;
    }
    if (try_launch_paged_attn<type_K, type_V, 1>(
            ctx, dst, scale, block_size, max_kv_seq_len)) {
        return;
    }
