...
SERVER_BIN="${SERVER_BIN:-$BUILD_DIR/luce_server}"
...
if [[ "$DYNAMIC_ROUTE_BALANCE" == 1 ]]; then
    if [[ "$DYNAMIC_MAIN_SLOTS" != auto ]]; then
        ...
        DYNAMIC_BALANCE_ENV_NAME="LUCE_MOE_TP_DYNAMIC_MAIN_SLOTS_X4"
        ...
        DYNAMIC_BALANCE_ENV_NAME="LUCE_MOE_TP_MAIN_TO_PEER_RATE"
...
if pgrep -f '(^|/)luce_server([[:space:]]|$)' >/dev/null; then
    echo "another luce_server is running; stop it before changing global GPU performance levels" >&2
    exit 2
fi
...
server_env=(
    ...
    "LUCE_CUDA_MMVQ_MOE_FP2_PACKED32=0"
    "LUCE_CUDA_MMVQ_FP4_X4=1"
    "LUCE_ROCMFP2_FIXED_K=1"
    "LUCE_ROCMFP3_FIXED_K=1"
    "LUCE_ROCMFP4_UNROLL2=1"
    "LUCE_MMID_GROUPED=1"
    "LUCE_MMID_GROUPED_TYPES=8"
    "LUCE_MMID_GROUPED_DEVICE=1"
    "LUCE_DS4_MOE_TP=1"
    "LUCE_DS4_MOE_TP_INPROC=1"
    "LUCE_DS4_MOE_TP_GPU=1"
    "LUCE_EXPERT_BUDGET_MB=$EXPERT_BUDGET_MB"
    "LUCE_DS4_HOTNESS_CSV=$HOTNESS_CSV"
    "LUCE_DS4_TP_CAPTURE_CACHE_SLOTS=4"
    "LUCE_DS4_TP_MASKED_ROUTES=1"
    "LUCE_DS4_TP_GROUPED_MMVQ=1"
    "LUCE_DS4_TP_SPLIT_COUNT=1"
    ...
    "LUCE_DS4_TP_COARSE_OWNER_SPLIT=0"
    ...
    "LUCE_DS4_PREFILL_TRACE=0"
    "LUCE_MOE_PREFILL_PERSISTENT_OWNER_ALLOC=1"
    "LUCE_DS4_PINNED_ROLLBACK=1"
    "LUCE_DS4_GPU_ARGMAX_VERIFY=1"
    "LUCE_DS4_SPEC=1"
    "LUCE_DS4_SPEC_Q=$VERIFY_WIDTH"
    "LUCE_DS4_ADAPTIVE_WIDTH=0"
    "LUCE_DS4_DRAFT=$DRAFT_MODEL"
    "LUCE_DS4_DRAFT_GPU=0"
    "LUCE_DS4_DRAFT_CONTEXT_KV_CACHE=1"
    "LUCE_MOE_FUSED_COMBINE=0"
)
...
if [[ -n "$DECODE_HOTNESS_CSV" ]]; then
    server_env+=(
        "LUCE_DS4_DECODE_HOTNESS_CSV=$DECODE_HOTNESS_CSV"
    )
fi
if [[ "$DYNAMIC_ROUTE_BALANCE" == 1 ]]; then
    server_env+=(
...
if [[ "$CACHE_SLOTS" != auto ]]; then
    server_env+=("LUCE_DS4_TP_FUSED_CACHE_SLOTS=$CACHE_SLOTS")
fi

if [[ "$FORCE_GRAPH_REPLAY" == 1 ]]; then
    server_env+=("LUCE_DS4_VERIFY_FORCE_GRAPH_REPLAY=1")
fi
if [[ "$SERIAL_INDEX_SCAN" == 1 ]]; then
    server_env+=("GGML_DS4_FA_SERIAL_INDEX_SCAN=1")
fi
if [[ "$DIRECT_INDEXER_TOPK" == 1 ]]; then
    server_env+=("LUCE_DS4_DIRECT_INDEXER_TOPK=1")
fi
...
if [[ "$Q5_VERIFY" == 1 ]]; then
    server_env+=("LUCE_DS4_Q5_VERIFY=1")
fi
if [[ "$FP4_Q5_X4_PLUS1" != auto ]]; then
    server_env+=("LUCE_CUDA_MMVQ_FP4_Q5_X4_PLUS1=$FP4_Q5_X4_PLUS1")
fi
if [[ "$CRITICAL_PATH_PLACEMENT" == 1 ]]; then
    server_env+=(
        "LUCE_DS4_TP_CRITICAL_PATH_PLACEMENT=1"
        "LUCE_DS4_TP_MAIN_TO_PEER_RATE=$MAIN_TO_PEER_RATE"
        "LUCE_DS4_TP_BALANCE_MIN_HOT=$BALANCE_MIN_HOT"
    )
fi
server_args=(
