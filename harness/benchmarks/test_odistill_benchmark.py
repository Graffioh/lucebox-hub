...
import odistill_benchmark as bench

sys.path.insert(0, str(Path(__file__).resolve().parent.parent))
import client_test_runner as client_bench


class ODistillBenchmarkTest(unittest.TestCase):
    @staticmethod
    def _report(phase, generation, case_id, accept, tps):
        return {
            "command": "bench",
            "model": "test-model",
            "odistill_phase": phase,
            "odistill_before": {"trainer_alive": False, "adapter_generation": generation},
            "odistill_after": {"trainer_alive": False, "adapter_generation": generation},
            "suites": {
                "he": {
    ...
    def test_phase_transition_requires_capture_without_drops(self):
        ...
        client_bench._validate_odistill_bench_transition(
            "heldout-adapted", before, after
        )
        with self.assertRaisesRegex(SystemExit, "no ODistill capture records"):
            client_bench._validate_odistill_bench_transition(
