#!/usr/bin/env python3
"""Prepare and compare leakage-resistant OFlash benchmark folds.
...
def _require_frozen_phase(report: dict[str, Any], expected: str, label: str) -> None:
    actual = report.get("oflash_phase")
    if actual != expected:
        raise ValueError(f"{label} report must use --oflash-phase {expected}, got {actual!r}")
    before = report.get("oflash_before") or {}
    after = report.get("oflash_after") or {}
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        "# OFlash frozen held-out comparison",
        "",
...
def cmd_prepare(args: argparse.Namespace) -> int:
    ...
    print(
        f"[oflash-bench] wrote {args.folds} folds with {manifest['total_cases']} cases "
        f"to {args.out_dir}"
    )
    print(f"[oflash-bench] manifest sha256={manifest['manifest_sha256']}")
    return 0
...
def cmd_compare(args: argparse.Namespace) -> int:
    ...
    summary = report["summary"]
    print(
        f"[oflash-bench] held-out n={summary['cases']} "
...
def cmd_pool(args: argparse.Namespace) -> int:
    ...
    if args.md_out:
        write_markdown(report, args.md_out)
    print(
        f"[oflash-bench] pooled folds={summary['folds']} n={summary['cases']} "
        f"accept_delta={_fmt(summary['mean_accept_rate_delta'])} "
        f"speedup={_fmt(summary['mean_speedup'], 3)}x"
    )
    parity_ok = summary["exact_output_matches"] == summary["cases"]
...
def build_parser() -> argparse.ArgumentParser:
    ...
    prepare.add_argument("--seed", default="oflash-v1")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"[oflash-bench] error: {exc}", file=sys.stderr)
        return 2
