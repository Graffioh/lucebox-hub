...
def verify(
    ...
    if "kvflash" in expected:
        ...
        if variant not in ("kvflash", "full"):
            # A report whose declared variant is inconsistent with a KVFlash
            # proof must not pass merely because its current prompts fit.
...
def main() -> int:
    ...
    parser.add_argument("--expect", action="append", choices=("ddtree", "pflash", "kvflash"), default=[])
    parser.add_argument("--out", type=Path, required=True)
    args = parser.parse_args()
    try:
        report = json.loads(args.bench.read_text(encoding="utf-8"))
        result = verify(report, parse_markers(args.server_log), set(args.expect))
