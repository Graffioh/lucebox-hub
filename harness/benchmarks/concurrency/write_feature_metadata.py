...
def main() -> int:
    ...
    parser.add_argument("--decode-mode", choices=("ar", "speculation", "adaptive"))
    parser.add_argument("--cache-type-k")
    parser.add_argument("--cache-type-v")
    parser.add_argument("--fa-window", type=int)
    parser.add_argument("--draft-always", choices=("on", "off"))
    parser.add_argument("--confidence", choices=("on", "off"))
    ...
    if args.decode_mode:
        literal_flags += ["--decode-mode", args.decode_mode]
    if args.cache_type_k:
        literal_flags += ["--cache-type-k", args.cache_type_k]
    if args.cache_type_v:
        literal_flags += ["--cache-type-v", args.cache_type_v]
    if args.fa_window is not None:
        literal_flags += ["--fa-window", str(args.fa_window)]
    if args.ddtree:
    ...
    obj = {
        ...
        "feature_config": {
            ...
            "draft_always": args.draft_always,
