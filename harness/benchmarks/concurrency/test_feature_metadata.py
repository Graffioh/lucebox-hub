...
class FeatureMetadataTests(unittest.TestCase):
    ...
    def test_runtime_cli_writes_the_requested_snapshot(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            metadata_file = root / "server-metadata.json"
            server_log = root / "server.log"
            out = root / "runtime-metadata.json"
            metadata_file.write_text(
                json.dumps({"schema_version": 3, "feature_config": {}}),
                encoding="utf-8",
            )
            server_log.write_text(
                "[paged-attention] 512 physical blocks x 16 tokens "
                "(8192 pool tokens, per-sequence max_ctx 4096)\n",
                encoding="utf-8",
            )
            argv = [
                str(RUNTIME_SCRIPT),
            ...
            with mock.patch.object(sys, "argv", argv):
                self.assertEqual(runtime_metadata.main(), 0)
            self.assertEqual(
