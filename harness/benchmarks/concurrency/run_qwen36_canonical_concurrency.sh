...
BLOG_GENERATOR="${BLOG_GENERATOR:-$SCRIPT_DIR/generate_blog_prompts.py}"
DDTREE_PROOF="${DDTREE_PROOF:-$SCRIPT_DIR/attach_ddtree_metrics.py}"
SUMMARIZER="${SUMMARIZER:-$SCRIPT_DIR/summarize_canonical_concurrency.py}"
MODEL="${MODEL:-}"
...
mkdir -p "$OUT/prompts"
python3 "$BLOG_GENERATOR" --out "$OUT/prompts/he-raw.jsonl"
...
run_case() {
  ...
  mkdir -p "$case_dir"
  local -a command launch client_common
  ...
  if [[ "$variant" == *ddtree ]]; then
    ...
    launch=(env DFLASH_IGNORE_EOS=1 DFLASH27B_DRAFT_SWA=2048 DFLASH_DDTREE_ADAPTIVE="$adaptive"
      DFLASH_MIN_TOKENS="$WARMUP_TOKENS" stdbuf -oL -eL "${command[@]}")
  else
    launch=(env DFLASH_IGNORE_EOS=1 DFLASH_MIN_TOKENS="$WARMUP_TOKENS"
...
import subprocess
out,variant,suite,c,repeat,binary,model,draft,prompts,cmd,n_gen,repo=sys.argv[1:]
...
pathlib.Path(out).write_text(json.dumps(obj,indent=2,sort_keys=True)+"\n")' \
  ...
  stop_server
  if (( status == 0 )) && [[ "$variant" == *ddtree ]]; then
    python3 "$DDTREE_PROOF" "$case_dir/bench.json" "$case_dir/server.log" \
      | tee "$case_dir/ddtree-proof.txt" || status=1
  fi
