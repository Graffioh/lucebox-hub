...
and decode runs one compact, power-of-two-bucketed step per token across the
live slots (the token axis of the graph becomes the sequence axis; GDN,
`ssm_conv`, and the paged kernel all iterate sequences independently — see
`ctest -R batched_gdn`). Admission only claims a slot and gates on the known
prompt size; K/V blocks are allocated as prefill chunks and decode tokens are
actually written, so `max_tokens` remains a generation limit rather than a
memory reservation. Prefill of a new request still runs synchronously between
decode iterations through a contiguous staging copy of its K/V (the exact
dense chunked path), dual-writing the same rows into its scattered pool
blocks. `--kv-pool-tokens` may size the shared pool below `N x max_ctx`.
...
3. **Done** (via staging): chunk prefill writes arbitrary block-table rows
   ...
   open.
4. **Partly done.** Claim-only admission, allocation on actual writes, compact
   live-sized decode buckets, and client-disconnect cancellation are in. Still
   open: prefill/decode interleaving (a prefill today pauses decode for its
   duration), oversubscription with preemption/recompute, and the stream-K
