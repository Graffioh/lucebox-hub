...
greedy output is byte-identical, and sampled runs continue from the exact RNG
position (recomputed logits can differ from the decode path within float
tolerance, so a near-tie may still diverge). After two preemptions a request
...
headroom while another sequence is active, clamped to half the pool so a
small explicit `--kv-pool-tokens` still co-admits instead of silently
serializing. Running prefill and decode may use that headroom.
...
4. **Done.** On-demand block allocation, memory-derived physical-pool sizing,
   ...
   recompute preemption under pressure. Still open: more than one in-flight
   prefill, and the stream-K decode kernel for ragged batches.
