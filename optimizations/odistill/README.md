...
<h1 align="center">Luce ODistill</h1>
...
<p align="center">
  ...
  ODistill captures what the verify step already computes, fine-tunes a LoRA on the idle iGPU, and
...
R9700 (gfx1201) serves · Strix Halo iGPU (gfx1151) trains
  ...
  reproduce: server/build/dflash_server target.gguf --draft drafter.gguf --draft-swa 2048 --odistill \
...
> states and target tokens at every draft position — and throws it away. ODistill keeps it: a capture
...
ODistill is the **online drafter adaptation layer** for the DFlash speculative-decode path in
[`../../server/`](../../server/) (qwen35 backend first). Design doc:
[`server/docs/ODISTILL.md`](../../server/docs/ODISTILL.md). It has two halves:

1. **Engine side** (`server/src/common/odistill/`): capture hook in the decode loop, SPSC shm ring,
...
[`odistill_format.h`](../../server/src/common/odistill/odistill_format.h) and mirrored in
[`ring_format.py`](src/odistill/ring_format.py) / [`adapter_export.py`](src/odistill/adapter_export.py),
...
┌───────────────────────────────┐      ┌─────────────────────────────────┐
│ Qwen35Backend decode loop     │      │ ODistill trainer sidecar (Python) │
│                               │      │                                 │
...
```bash
HIP_VISIBLE_DEVICES=1 optimizations/odistill/.venv/bin/python - <<'PY'
import torch
...
PY
HIP_VISIBLE_DEVICES=1 optimizations/odistill/.venv/bin/python -m torch.utils.collect_env
```
...
server/build/dflash_server \
  server/models/odistill/Qwen3.6-27B-Q4_K_M.gguf \
...
[`server/docs/ODISTILL.md`](../../server/docs/ODISTILL.md#r9700-and-strix-halo-qualification-ladder)
before enabling the integrated sidecar. The engine passes the target path to an integrated trainer;
`--target`, `$ODISTILL_TARGET_GGUF`, or `trainer.json` is needed only for direct trainer invocation.
...
- the Q4_K_M file selected here is the R9700 reference artifact. Strix used Q8_0, so its number is
  capacity/performance evidence rather than an ODistill baseline.
...
`~/.lucebox/odistill/<drafter-hash>/<profile>-sem-<contract-hash>/`; the next session warm-starts
...
| Flag | Purpose |
|---|---|
| `--odistill` | enable online drafter adaptation |
| `--odistill-device <cpu\|N>` | trainer sidecar device: `cpu` or a HIP ordinal (default: `1`, the iGPU) |
| `--odistill-dtype <type>` | mirror dtype: `auto`, `fp16`, `bf16`, or `fp32` (default: `auto`; GPU = fp16, CPU = fp32) |
...
| `--odistill-topk <K>` | target top-K captured per position (default: 8; 0 = skip) |
| `--odistill-trainer-bin <path>` | trainer sidecar executable; empty = capture-only (M0 telemetry) mode |

## Trainer knobs

`bin/odistill-trainer <drafter.gguf> ...` — the engine passes the wiring arguments itself
...
[`ODISTILL.md`](../../server/docs/ODISTILL.md#usage)):

| Flag | Default | Purpose |
|---|---|---|
| `--target <path>` | trainer.json / `$ODISTILL_TARGET_GGUF` | target GGUF for lm_head + token_embd |
...
- [**Draft-OPD**](https://arxiv.org/abs/2605.29343): target-assisted rollouts and error-position
  replay. ODistill does not yet retain the rejected suffix required to implement its reverse-KL term
...
[`ODISTILL.md`](../../server/docs/ODISTILL.md):
...
- **Persistent, single-device decode drafter.** ODistill is rejected with PFlash compression,
...
- **There is no implicit GPU-to-CPU fallback.** If the requested HIP device or accelerator preflight
  ...
  `--odistill-device cpu` explicitly if CPU training is desired, with the same host-RAM guards.

## Citation

```bibtex
@software{luce_odistill_2026,
  title  = {Luce ODistill: online drafter adaptation for speculative decoding on iGPU+dGPU boxes},
  author = {Lucebox},
  url    = {https://github.com/Luce-Org/lucebox-hub/tree/main/optimizations/odistill},
  year   = {2026}
}
