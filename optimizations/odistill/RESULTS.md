...
[`R9700 and Strix Halo qualification ladder`](../../server/docs/ODISTILL.md#r9700-and-strix-halo-qualification-ladder).

The planned ODistill measurements use one box: **AMD Radeon AI PRO R9700** (gfx1201, 32 GB GDDR6)
...
work concurrently. They are not ODistill measurements and must not be combined into a synthetic
...
32 GiB R9700 VRAM and 127 GiB across KFD allocations. ODistill was stopped instead of competing past
...
overhead itself (target tok/s with vs without `--odistill`; budget < 1%). This baseline is what every
...
server/build/dflash_server qwen36-27b-Q4_K_M.gguf \
    ...
    --ddtree --ddtree-budget 22 \
    --odistill                                               # no trainer bin: capture-only

python3 harness/benchmarks/generation_benchmark.py run \
    --name odistill-m0-he --url http://127.0.0.1:8080/v1 --model qwen36 \
...
acceptance rates 0.15625, 0.31250 and 0.51042. ODistill-off and capture-only outputs were
byte-identical, `records_dropped=0`, and the R9700 allocation was about 19.25 GiB.

For three warmed repeats of the 111-token code prompt, ODistill-off measured 90.7/90.7/90.7 tok/s
...
ODISTILL_PROFILE_DIR=/path/from/odistill-profile-dir-log
ODISTILL_RING_NAME=/lucebox-odistill-12345
optimizations/odistill/bin/odistill-trainer \
    ...
    --out-dir "$ODISTILL_PROFILE_DIR" --profile default \
...
# promoted in "$ODISTILL_PROFILE_DIR/promoted.json", then restart the M0 server command.
...
Use `harness/benchmarks/odistill_benchmark.py` for this gate. Its default three-fold split uses the
...
`harness/client_test_runner.py bench --odistill-phase ...` rejects a live trainer or generation change
...
drops. The generation-20 adapter was 13.1 MB and retained the complete format-v3 model/semantic
...
server/build/dflash_server qwen36-27b-Q4_K_M.gguf \
    ...
    --max-ctx 4096 --cache-type-k q4_0 --cache-type-v q4_0 \
    --ddtree --ddtree-budget 22 \
    --odistill --odistill-trainer-bin optimizations/odistill/bin/odistill-trainer
...
# ~/.lucebox/odistill/<hash16>/default-sem-<contract-hash>/promoted.json
```
...
controller between iGPU and CPU). This decides the default `--odistill-device` and is the publishable
systems measurement.

```bash
# three runs of the M2 command, varying:
#   --odistill-device 1        (iGPU)
#   --odistill-device cpu
#   (--odistill omitted)       (baseline)
```
