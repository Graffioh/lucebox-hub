"""odistill-trainer entry point — the sidecar the engine spawns.

Process contract (server/src/common/odistill/odistill_supervisor.cpp):

  argv:   odistill-trainer <drafter.gguf>
              --ring-name=/lucebox-odistill-<pid>  --out-dir=<profile dir>
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    # Training knobs (ODISTILL.md §5 defaults).
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        sys.stderr.write(f"[odistill-trainer] ring attach failed: {e}\n")
        if args.stream_fd >= 0:
            os.write(args.stream_fd, struct.pack("<i", -1))
        return 1

    target = args.target or os.environ.get("ODISTILL_TARGET_GGUF", "")
    try:
        if not target:
        ...
        sys.stderr.write(f"[odistill-trainer] target config failed: {e}\n")
        if args.stream_fd >= 0:
            os.write(args.stream_fd, struct.pack("<i", -1))
        ring.close()
        return 1
    if not isinstance(target, str) or not target or not os.path.exists(target):
        sys.stderr.write(
            "[odistill-trainer] no target GGUF configured (--target / "
            "$ODISTILL_TARGET_GGUF / trainer.json target_gguf)\n")
        if args.stream_fd >= 0:
            os.write(args.stream_fd, struct.pack("<i", -1))
        ring.close()
        return 1
    if not os.path.exists(args.drafter):
        sys.stderr.write(
            f"[odistill-trainer] drafter GGUF not found: {args.drafter}\n")
        if args.stream_fd >= 0:
            os.write(args.stream_fd, struct.pack("<i", -1))
        ring.close()
        return 1
    if args.train_ctx < 64:  # keep in sync with trainer.MIN_WINDOW_ROWS
        sys.stderr.write(
            f"[odistill-trainer] --train-ctx {args.train_ctx} must be >= 64\n")
        if args.stream_fd >= 0:
            os.write(args.stream_fd, struct.pack("<i", -1))
        ring.close()
        return 1
    if args.batch_rows <= 0 or args.export_every <= 0 \
        ...
        sys.stderr.write(
            "[odistill-trainer] batch/export/reservoir/generation limits must "
            "be positive\n")
        if args.stream_fd >= 0:
            os.write(args.stream_fd, struct.pack("<i", -1))
        ring.close()
    ...
    try:
        ...
        for label, path, supplied_digest in supplied:
            if args.stream_fd >= 0:
                continue
            sys.stderr.write(
                f"[odistill-trainer] verifying {label} GGUF for adapter "
                f"identity: {path}\n")
        ...
        sys.stderr.write(f"[odistill-trainer] model identity failed: {e}\n")
        if args.stream_fd >= 0:
            os.write(args.stream_fd, struct.pack("<i", -1))
        ring.close()
        return 1
