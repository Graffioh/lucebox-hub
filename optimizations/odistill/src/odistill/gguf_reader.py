"""GGUF loading + dequantization for the ODistill trainer sidecar.

The trainer builds its low-precision drafter mirror by dequantizing the same GGUFs the
engine serves (ODISTILL.md §11/§12) — no safetensors drafter ships on this box.
...
Requires the `gguf` package (>= 0.10; the odistill[train] extra). Imports are
