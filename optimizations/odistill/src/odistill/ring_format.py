"""Python mirror of the ODistill capture-ring format.

Single source of truth: server/src/common/odistill/odistill_format.h. Any change
there bumps RING_VERSION and must be mirrored here; the golden-bytes tests on
both sides (server/test/test_odistill_unit.cpp ring_step_record_golden_bytes and
...
RING_MAGIC = 0x4F444931  # "ODI1"
RING_VERSION = 3
HEADER_SIZE = 256
RECORD_HEADER_SIZE = 32

# Header field offsets (bytes) — see odistill_format.h layout comment.
OFF_MAGIC = 0
OFF_VERSION = 4
OFF_CAPACITY = 8
OFF_DATA_OFFSET = 16
...
# ── Test-only writer (mirrors ODistillRing::push2) ─────────────────────
